...
async function waitForServer(url, timeoutMs = 30_000) {
  ...
  throw new Error(`preview server never answered at ${url}: ${lastErr?.message ?? 'timed out'}`);
}
...
function findChromiumExecutable() {
  ...
  const fallback = '/home/bewinxed/.cache/ms-playwright/chromium-1234/chrome-linux64/chrome';
...
function startFrontProxy({ port, previewPort, hub }) {
  ...
  const to = (req) =>
    /^\/(ws|api)(\/|$)/.test(req.url ?? '')
      ...
      : { host: '127.0.0.1', port: previewPort, hostHeader: `127.0.0.1:${previewPort}` };
...
try {
  ...
  previewProc = spawn('bun', ['run', 'preview', '--', '--port', String(previewPort), '--strictPort'], {
    cwd: dashboardDir,
  ...
  previewProc.on('exit', (code, signal) => {
    if (code !== null && code !== 0) {
      console.error(`[insecure-origin-smoke] preview server exited early (code ${code}, signal ${signal})`);
  ...
  console.log(`[insecure-origin-smoke] window.isSecureContext === ${isSecureContext}`);
  ...
  if (composerVisible) {
    ...
    console.log('[insecure-origin-smoke] composer reached: typed and submitted with no throw');
  } else {
    console.log(
  ...
  if (!sessionId) {
    ...
    console.log(`[insecure-origin-smoke] failed-ghost: opening session ${sessionId}`);
    await page.goto(`http://${INSECURE_HOST}:${port}/session/${sessionId}`, {
      waitUntil: 'domcontentloaded',
      timeout: 30_000,
    });
    const field = page.locator('textarea[aria-label="Message the agent"]').first();
    ...
    // cause on the spot.
    await field.waitFor({ state: 'visible', timeout: 20_000 }).catch(async () => {
      const pane = await page
        .evaluate(() => ({
          hub: document.querySelector('[data-hub-status]')?.textContent ?? null,
          text: (document.querySelector('.pane')?.textContent ?? document.body.innerText).slice(0, 400),
        }))
        .catch(() => null);
      throw new Error(
    ...
    for (let attempt = 0; attempt < 5 && !filled; attempt += 1) {
      filled = await field
        .fill(GHOST_PROBE_TEXT, { timeout: 5_000 })
        .then(() => true)
        .catch(() => false);
    ...
    const ghost = page.locator('section.turn.you.failed').filter({ hasText: GHOST_PROBE_TEXT });
    await ghost
      .first()
      ...
      .catch(async () => {
        ...
        const rows = await page
          .evaluate((probe) => ({
            userRows: [...document.querySelectorAll('section.turn.you')].map((r) => ({
              cls: r.className,
    ...
    const rendered = await ghost.first().evaluate((row) => ({
      ...
      actions: [...row.querySelectorAll('button')].map((b) => b.textContent.trim()),
    }));
    ...
    if (!rendered.reason?.startsWith("Couldn't send that message.")) {
      throw new Error(`failed ghost carries no reason line (got ${JSON.stringify(rendered.reason)})`);
    }
    ...
    if (!live.includes('Message not sent')) {
      throw new Error(`the live region never announced the failure (read ${JSON.stringify(live)})`);
    }
    console.log(
  ...
  if (pageErrors.length > 0) {
    console.error(`[insecure-origin-smoke] ${pageErrors.length} uncaught page error(s):`);
    for (const err of pageErrors) console.error(`  - ${err.message}`);
    throw new Error('page threw uncaught exception(s) on an insecure origin');
  }

  console.log('[insecure-origin-smoke] PASS — insecure origin, zero uncaught page errors');
} catch (err) {
