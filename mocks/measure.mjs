...
const page = await browser.newPage({ viewport: { width: 1440, height: 1023 }, deviceScaleFactor: 2 });
await page.goto(pathToFileURL(join(HERE, 'v2-fleet.html')).href, { waitUntil: 'load' });
...
const stops = (action.backgroundImage.match(/(?:rgb|oklch|color)\([^)]*\)/g) || []);
check('action carries a linear-gradient (not a flat fill)',
  action.backgroundImage.includes('linear-gradient'), action.backgroundImage.slice(0, 40) + '…');
check('gradient has two distinct stops (top highlight -> body)',
  stops.length >= 2 && stops[0] !== stops[1], `${stops[0]} -> ${stops[1]}`);
check('action carries an inset bottom edge',
  action.boxShadow.includes('inset'), action.boxShadow.split(',').find((s) => s.includes('inset'))?.trim());
...
check('border scale is graded, not flattened to one value',
  uniq.size === 3, `${uniq.size} distinct values`);
...
const shapes = await page.evaluate(() => {
  const g = (sel) => {
    ...
    const cs = getComputedStyle(el);
    const r = el.getBoundingClientRect();
    return { radius: cs.borderRadius, w: +r.width.toFixed(1), h: +r.height.toFixed(1),
             bg: cs.backgroundColor, bgi: cs.backgroundImage.slice(0, 30) };
  };
  return { panel: g('.panel'), card: g('.stat'), well: g('.stat .well'),
           tile: g('.stat .chip'), mark: g('.nm-cell .mark') };
});
for (const [k, v] of Object.entries(shapes)) {
  console.log(`     ${k.padEnd(6)} ${String(v && v.w).padStart(6)}x${String(v && v.h).padEnd(6)} r=${v && v.radius}`);
}
check('table panel radius 14px', shapes.panel?.radius === '14px', shapes.panel?.radius);
check('stat card radius 10px', shapes.card?.radius === '10px', shapes.card?.radius);
check('stat well radius 7px', shapes.well?.radius === '7px', shapes.well?.radius);
check('stat icon tile 24x24 r5.5', shapes.tile?.w === 24 && shapes.tile?.h === 24
  && shapes.tile?.radius === '5.5px', `${shapes.tile?.w}x${shapes.tile?.h} r=${shapes.tile?.radius}`);
check('row mark 17x17 r4.6 with a gradient fill (not a hue fill)',
  shapes.mark?.w === 17 && shapes.mark?.h === 17 && shapes.mark?.radius === '4.6px'
...
const chips = await page.evaluate(() => [...document.querySelectorAll('.chip-s')].map((el) => ({
  cls: [...el.classList].find((c) => c.startsWith('s-')),
  label: el.textContent.trim(),
  glyph: el.querySelector('svg')?.querySelector('path,circle')?.getAttribute('d')
      || el.querySelector('svg')?.tagName || null,
  fill: getComputedStyle(el).backgroundColor,
})));
const states = new Map();
for (const c of chips) states.set(c.cls, c);
for (const [cls, c] of states) console.log(`     ${cls.padEnd(8)} "${c.label}"  glyph=${c.glyph ? 'yes' : 'NONE'}  fill=${c.fill}`);
check('every status chip carries a glyph as well as a label',
  chips.length > 0 && chips.every((c) => c.glyph), `${chips.length} chips`);
check('every status state has a distinct glyph',
  new Set([...states.values()].map((c) => c.glyph)).size === states.size,
  `${states.size} states`);
check('idle ships no fill (measured 6.7 dE from the live chip when it had one)',
...
const themed = await page.evaluate(() => {
  ...
  const hex = (v) => {
    ...
    try { ctx.fillStyle = v; } catch { return v; }
  ...
  const read = () => {
    ...
    probe.remove();
    return { body: hex(cs.backgroundColor), ink: hex(cs.color), neutral1: hex(n1) };
  };
  const light = read();
...
console.log(`     light: body-bg ${themed.light.body}  ink ${themed.light.ink}  --neutral-1 ${themed.light.neutral1}`);
console.log(`     dark : body-bg ${themed.dark.body}  ink ${themed.dark.ink}  --neutral-1 ${themed.dark.neutral1}`);
const lum = (h) => { const n = h.replace('#', ''); return [0, 2, 4].reduce((a, i) => a + parseInt(n.slice(i, i + 2), 16), 0) / 3; };
check('.dark flips the page ground from light to dark',
  lum(themed.light.body) > 200 && lum(themed.dark.body) < 60,
  `${lum(themed.light.body).toFixed(0)} -> ${lum(themed.dark.body).toFixed(0)}`);
check('.dark flips the ink from dark to light',
  lum(themed.light.ink) < 80 && lum(themed.dark.ink) > 180,
  `${lum(themed.light.ink).toFixed(0)} -> ${lum(themed.dark.ink).toFixed(0)}`);
check('.dark re-solves the ramp (neutral-1 is a different value, not an inversion)',
