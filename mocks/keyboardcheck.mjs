...
for (const file of FILES) {
  ...
  for (const dark of [false, true]) {
    const page = await browser.newPage({ viewport: { width, height: 1023 }, deviceScaleFactor: 2 });
    ...
    const inventory = await page.evaluate(() => {
      ...
      const pointer = [], unreachable = [], unnamed = [];
      ...
      for (const el of document.querySelectorAll('body *')) {
        ...
        const actsLikeControl = cs.cursor === 'pointer'
          || el.hasAttribute('onclick') || ['button', 'link', 'tab', 'menuitem', 'checkbox', 'switch']
            .includes(el.getAttribute('role'));
        if (!actsLikeControl) continue;
        // a pointer affordance whose only job is to host a focusable child is fine
        if (el.querySelector(FOCUSABLE)) continue;
        if (el.closest(FOCUSABLE)) continue;   // the focusable ancestor is the control
        const label = (el.tagName + '.' + cls(el)).slice(0, 30)
          + ' "' + name(el).slice(0, 18) + '"';
        pointer.push(label);
        if (!el.matches(FOCUSABLE)) unreachable.push(label);
        else if (!name(el)) unnamed.push(label);
      }
      return { pointerCount: pointer.length, unreachable, unnamed,
    ...
    for (let i = 0; i < MAX_TABS; i++) {
      ...
      const stop = await page.evaluate(() => {
        ...
        return {
          ...
          region: (() => {
            const l = el.closest('aside,main,header,footer,nav,section,[role="dialog"]');
            return l ? l.tagName + '.' + String(l.className || '').slice(0, 12) : 'root';
          })(),
          outline: cs.outlineStyle === 'none' ? null : `${cs.outlineWidth} ${cs.outlineColor}`,
        };
      });
      if (!stop) break;
      if (stops.length && stops[stops.length - 1].tag === stop.tag
        && stops[stops.length - 1].x === stop.x && stops[stops.length - 1].y === stop.y) break;
      stops.push(stop);
    }

    // ---- 3: is the ring actually painted and distinguishable? ---------------
    let ringFails = 0, ringMin = 99;
    if (stops.length) {
      await page.evaluate(() => document.body.focus());
      ...
      if (probe) {
        ...
        if (clip.width > 4 && clip.height > 4
          ...
          const diff = await page.evaluate(async ({ a, b }) => {
            ...
            let changed = 0, ringPx = null, bgPx = null, best = 0;
            for (let i = 0; i < A.length; i += 4) {
              ...
              if (d > 24) {
                ...
                if (d > best) { best = d; ringPx = [A[i], A[i + 1], A[i + 2]]; bgPx = [B[i], B[i + 1], B[i + 2]]; }
              }
            }
            return { changed, ringPx, bgPx };
          }, { a: withRing.toString('base64'), b: without.toString('base64') });
          if (!diff.changed) { ringFails++; }
    ...
    for (let i = 1; i < stops.length; i++) {
      ...
      if (a.region !== b.region) continue;             // region change is legitimate
      if (b.y + b.h <= a.y - 1) inversions++;          // jumped upward a full row
    }

    // is there a keyboard-operable way to REACH the navigation at this width?
    const opener = await page.evaluate(() => {
      const aside = document.querySelector('aside');
      if (!aside) return { needed: false };
      const hidden = aside.getBoundingClientRect().right <= 0
        || getComputedStyle(aside).transform.includes('-2')
        || getComputedStyle(aside).display === 'none';
      if (!hidden) return { needed: false };
      const FOCUSABLE = 'a[href],button,input:not([hidden]),select,textarea,[tabindex]:not([tabindex="-1"])';
      const candidates = [...document.querySelectorAll('.burger,[aria-controls],[data-drawer]')];
      const usable = candidates.find((el) => el.matches(FOCUSABLE)
        && el.getBoundingClientRect().width > 0 && el.tabIndex >= 0);
      return { needed: true, present: !!usable,
        found: candidates.map((c) => `${c.tagName}.${c.className} tabIndex=${c.tabIndex}`) };
    });
    ...
    for (const sel of ['.nav-i:not(.on)', 'tbody tr td']) {
      ...
      const after = await target.evaluate((e) => {
        ...
        return { bg: cs.backgroundColor, bgPx: px(cs.backgroundColor), inkPx: px(cs.color) };
      });
      if (before === after.bg) stateProblems.push(`${sel} hover changes nothing`);
      else {
        const r = ratio(after.inkPx, after.bgPx);
        if (r < 4.5) stateProblems.push(`${sel} ink on hover surface ${r.toFixed(2)}:1`);
      }
    ...
    if (opener.needed && !opener.present) {
      problems.push('drawer is closed and has NO keyboard-focusable opener '
        + `(${opener.found.join('; ') || 'no candidate'})`);
    }
    if (offscreen.length) {
      problems.push(`${offscreen.length} tab stop(s) land off-screen `
        + `(first at x=${offscreen[0].x}, "${offscreen[0].name}")`);
    }
    if (inventory.unreachable.length) {
      problems.push(`${inventory.unreachable.length} of ${inventory.pointerCount} pointer affordances are not focusable`);
    }
    if (inventory.unnamed.length) problems.push(`${inventory.unnamed.length} focusable control(s) with no accessible name`);
    if (ringFails) problems.push('focus indicator paints nothing');
    if (ringMin < 3 && ringMin !== 99) problems.push(`focus ring ${ringMin.toFixed(2)}:1 against its background (need 3:1)`);
    if (inversions) problems.push(`${inversions} tab-order inversion(s) against reading order`);
    for (const sp of stateProblems) problems.push(sp);

    console.log(`  ${problems.length ? 'FAIL' : 'PASS'}  ${tag.padEnd(30)}`
    ...
    if (inventory.unreachable.length) {
      for (const u of [...new Set(inventory.unreachable)].slice(0, 5)) console.log(`            not focusable: ${u}`);
    }
    await page.close();
  }
  }
...
await browser.close();
console.log(fails ? `  ${fails} keyboard/focus failures`
