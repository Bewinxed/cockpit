...
const browser = await chromium.launch({
  executablePath: process.env.CHROMIUM_BIN || '/home/bewinxed/.cache/ms-playwright/chromium-1234/chrome-linux64/chrome',
...
const rect = (p, s) => p.evaluate((sel) => { const e = document.querySelector(sel); if (!e) return null; const r = e.getBoundingClientRect(); return { x: r.x, y: r.y, w: r.width, h: r.height, right: r.right }; }, s);

// ================= DW-9b.1 — the preserved shell is LAW =================
console.log('== DW-9b.1: preserved shell (380x899, 24/40, radius 16, header 47, pitch 44, scrim .06) ==');
{
  const p = await open();
  ...
  ok(asst && near(iw - asst.right, 24), `panel inset right 24 (got ${asst && (iw - asst.right)})`);
  const radius = await p.evaluate(() => getComputedStyle(document.querySelector('.asst')).borderTopLeftRadius);
  ...
  ok(sugH && near(sugH.h, 40), `suggestion-row height 40 (got ${sugH && sugH.h})`);
  // scrim alpha ~ .06 (rgba(0,0,0,.06) == --scrim-soft), light + dark both present
  const alpha = await p.evaluate(() => {
    const bg = getComputedStyle(document.querySelector('.asst-scrim')).backgroundColor;
    const m = bg.match(/[\d.]+/g); if (!m) return null;
    return m.length >= 4 ? parseFloat(m[3]) : 1; // rgba alpha; oklch(.. / a) also lands last
  });
  ok(alpha !== null && alpha > 0 && alpha <= 0.12, `scrim is a light wash, alpha<=.12 (got ${alpha})`);
  await p.close();
  // dark scheme: shell geometry identical
  const pd = await open({ dark: true });
  const ad = await rect(pd, '.asst');
  ok(ad && near(ad.w, 380) && near(ad.h, 899), `panel 380x899 unchanged in dark (${ad && ad.w}x${ad && ad.h})`);
  await pd.close();
}

// ================= DW-9b.2 — action->undo/checkpoint, zero silent =================
console.log('== DW-9b.2: action contract, every action -> undo/checkpoint, zero silent ==');
{
  const p = await open();
  // visible action list: every row has a reversibility tag, all in {undo,checkpoint}, none silent
  const acts = await p.evaluate(() => [...document.querySelectorAll('.a-can li')].map(li => ({
  ...
  ok(acts.every(a => a.rv === 'undo' || a.rv === 'checkpoint'), 'every action tag is undo|checkpoint');
  ok(acts.every(a => a.rv !== 'silent'), 'zero actions map to silent');
  ok(acts.some(a => a.rv === 'checkpoint') && acts.some(a => a.rv === 'undo'), 'both undo and checkpoint appear');
  await p.close();
  ...
  ok(/\[card\]/.test(cmt) && /approval card/i.test(raw), 'consequential actions carry the approval card');
  ok(/checkpoint/.test(cmt) && /undo/.test(cmt), 'contract maps actions to undo/checkpoint');
  ok(/inherited whole|inherits Phase 5|approval card \(symmetric/i.test(raw), 'Phase 5 approval contract inherited in full');
}

// ================= DW-9b.3 — distinguishable from a transcript, non-positional =================
console.log('== DW-9b.3: distinguishable from a session transcript, not by position alone ==');
{
  const p = await open();
  ...
  const trRole = await p.evaluate(() => [...document.querySelectorAll('.tr .who')].map(w => w.textContent.trim()));
  ok(trRole.some(r => /Claude Code/i.test(r)) && !/Assistant/i.test(trRole.join('|')),
  ...
  ok(headerHasCue, 'the name + role cue live in the always-visible header (non-positional)');
  await p.close();
}

// ================= DW-9b.4 — ai-native canon-gap labels =================
console.log('== DW-9b.4: every recommendation principle-derived; 4 canon gaps marked inference ==');
{
  ok(/principle-derived/i.test(raw), 'recommendations labelled principle-derived (ai-native has no settled canon)');
