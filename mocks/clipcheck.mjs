...
const LEAD_MIN = 1.2, LEAD_MAX = 1.4, EPS = 0.005;
...
let clipped = 0, escaped = 0, badLead = 0, skipped = 0;
...
for (const file of FILES) {
  for (const width of WIDTHS) {
    ...
    for (const dark of [false, true]) {
      ...
      const r = await page.evaluate(({ LEAD_MIN, LEAD_MAX, EPS }) => {
        const out = { clipped: [], escaped: [], lead: {}, skipped: [] };
        const numericLead = parseFloat(
          getComputedStyle(document.documentElement).getPropertyValue('--leading-numeric')) || null;
        ...
        const lineBox = (el) => {
          ...
          for (const n of el.childNodes) {
            ...
            for (const x of rng.getClientRects()) best = Math.max(best, x.height);
            rng.detach?.();
          }
          return best || null;
        };
        ...
        for (const el of document.querySelectorAll('body *')) {
          ...
          if (!hasText) continue;
          const cs = getComputedStyle(el);
          const name = (el.tagName + '.' + String(el.className || '')).slice(0, 44);
          ...
          if ((hidY && el.scrollHeight > el.clientHeight + 1)
            || (hidX && !ellipsis && el.scrollWidth > el.clientWidth + 1)) {
            out.clipped.push({ name, ch: el.clientHeight, sh: el.scrollHeight,
              cw: el.clientWidth, sw: el.scrollWidth, fs: cs.fontSize, lh: cs.lineHeight });
          }
          ...
          for (let anc = el.parentElement; anc && anc !== document.body; anc = anc.parentElement) {
            ...
            if (acs.overflow !== 'visible' || acs.overflowY !== 'visible') break;
            const constrained = anc.scrollHeight > anc.clientHeight + 1
              || acs.maxHeight !== 'none';
            if (!constrained) continue;
            const cb = contentBox(anc);
            if (er.bottom > cb.bottom + 1 || er.top < cb.top - 1) {
              out.escaped.push({ name,
                anc: (anc.tagName + '.' + String(anc.className || '')).slice(0, 30),
                by: +Math.max(er.bottom - cb.bottom, cb.top - er.top).toFixed(1) });
            }
            break;
          }
          ...
          let how = 'computed';
          if (!(lh > 0)) {                     // `normal` — measure the line box
            lh = lineBox(el);
            how = 'measured';
          }
          if (fs > 0 && lh > 0) {
            ...
            const isNumericToken = numericLead !== null
              && Math.abs(ratio - numericLead) <= EPS;
            if (!isNumericToken && (ratio < LEAD_MIN - EPS || ratio > LEAD_MAX + EPS)) {
      ...
      for (const c of r.clipped) {
        clipped++;
        console.log(`    CLIPPED  ${tag}  ${c.name}  client ${c.cw}x${c.ch} vs scroll ${c.sw}x${c.sh}  ${c.fs}/${c.lh}`);
      }
      for (const e of r.escaped) {
        escaped++;
        console.log(`    ESCAPED  ${tag}  ${e.name} out of ${e.anc} content box by ${e.by}px`);
      }
      for (const [k, n] of Object.entries(r.lead)) {
        if (k.startsWith('BAD:')) {
          badLead += n;
          console.log(`    LEADING  ${tag}  ratio ${k.slice(4)} on ${n} element(s) — outside ${LEAD_MIN}-${LEAD_MAX}`);
        } else {
      ...
      for (const s of r.skipped) {
        skipped++;
        skipList.set(`${s.name} (${s.why})`, (skipList.get(`${s.name} (${s.why})`) || 0) + 1);
      }
      await page.close();
    }
    }
...
console.log(`    computed leading ratios seen: `
  + [...seenLead.entries()].sort((a, b) => b[1] - a[1]).slice(0, 8)
    .map(([k, n]) => `${k}x${n}`).join('  ') + '   (* = measured line box)');
if (skipList.size) {
