...
// ---------------------------------------------------------------------------
console.log(
  "== DW-8.4 row 1 — every :hover wrapped in (hover:hover) and (pointer:fine) =="
);
{
  ...
  let inGated = 0;
  for (const [a, b] of markers) {
    inGated += countHover(css.slice(a, b));
  }
  const outside = countHover(remainder);
  ok(
    markers.length >= 1,
    `DW-8.4.1 at least one gated hover media block present (${markers.length})`
  );
  ok(
    outside === 0,
    `DW-8.4.1 zero :hover rules OUTSIDE the (hover:hover)+(pointer:fine) gate (${outside} found)`
  );
  ok(
    inGated >= 4,
    `DW-8.4.1 ${inGated} :hover rules gated under (hover:hover) and (pointer:fine)`
  );
}
...
{
  ok(
    headText.includes("viewport-fit=cover"),
    "DW-8.4.7 viewport-fit=cover declared in the viewport meta"
  );
  ok(
    /env\(\s*safe-area-inset-(?:top|right|bottom|left)\s*\)/.test(styleText),
  ...
  ok(
    tc.length >= 2,
    `DW-8.4.9 two theme-color metas declared for light+dark (${tc.length})`
  );
  ok(
    tc.some((t) => t.includes("prefers-color-scheme: dark")),
...
}
{
  ...
  const noBare = css
    .replace(/\d+\.?\d*(dvh|svh)\b/g, "")
    .replace(/[%;\s,()]/g, " ");
  const bare = (noBare.match(/\bvh\b/g) || []).length;
  ok(
    bare === 0,
    `DW-8.4.3 no bare vh unit (only dvh/svh); ${bare} bare vh found`
  );
  ok(
    /\d+dvh\b/.test(css) && /100dvh/.test(css),
    "DW-8.4.3 app shell uses 100dvh"
  );
  ok(
    /\d+svh\b/.test(css),
    "DW-8.4.3 a full-bleed block uses svh (100svh for full-bleed)"
  );
  ok(
    /overscroll-behavior\s*:\s*none/.test(css),
    "DW-8.4.6 overscroll-behavior:none declared"
  );
  const ob = css.match(/overscroll-behavior[^;}]*[;}]/g) || [];
  ok(
    ob.length >= 2,
    `DW-8.4.6 overscroll-behavior:none on html AND body (${ob.length} declarations)`
  );
}
...
async function open({
  ...
  await page.goto(pathToFileURL(join(HERE, "v5-workspace.html")).href, {
    waitUntil: "load",
  });
  if (scheme === "dark") {
...
console.log("== DW-8.1 / 8.9 — tab reorder via CSS order; reachable set ==");
{
  ...
  const domDefault = await p.evaluate(() =>
    window.__domOrder(".tabstrip .tab")
  );
  // default visual order = DOM order (orders 1..4)
  const visDefault = await p.evaluate(() =>
    window.__computedOrder(".tabstrip .tab")
  );
  ok(
    JSON.stringify(domDefault) === JSON.stringify(visDefault),
  ...
  await p.evaluate(() => {
    document.body.setAttribute("data-ws", "reordered");
    document.querySelector(".tabstrip").setAttribute("data-order", "reordered");
  });
  await p.waitForTimeout(30);
  const domAfter = await p.evaluate(() => window.__domOrder(".tabstrip .tab"));
  const visAfter = await p.evaluate(() =>
    window.__computedOrder(".tabstrip .tab")
  );
  ok(
    JSON.stringify(domBefore) === JSON.stringify(domAfter),
    `DW-8.1 DOM order unchanged across reorder (${domBefore.join(",")} stays)`
  );
  ok(
    JSON.stringify(visDefault) !== JSON.stringify(visAfter),
  ...
  const orders = await p.evaluate(() =>
    Array.from(document.querySelectorAll(".tabstrip .tab")).map(
      (e) => getComputedStyle(e).order
    )
  );
  ok(
    new Set(orders).size === orders.length,
    `DW-8.1 tab visual order driven by CSS order values (${orders.join(",")})`
  );
  // DW-8.9: reachable set = open tabs in the strip
  const openTabs = await p.evaluate(() =>
    document.querySelector(".tabstrip").getAttribute("data-open-tabs")
  );
  const renderedTabs = (
    await p.evaluate(() => window.__domOrder(".tabstrip .tab"))
  ).join(",");
  ok(
    openTabs && openTabs.split(",").length === renderedTabs.split(",").length,
  ...
  const sidebarRuns = (
    await p.evaluate(() =>
      Array.from(document.querySelectorAll(".runs .run-i")).map(
        (e) => e.textContent
      )
    )
  ).length;
  ok(
    sidebarRuns >= 2,
    `DW-8.9 non-open sessions exist in the sidebar Running now but are NOT in the strip (${sidebarRuns})`
  );
  const tabCount = (await p.evaluate(() => window.__domOrder(".tabstrip .tab")))
    .length;
  const allSessions = tabCount + sidebarRuns;
  ok(
    allSessions > tabCount,
    `DW-8.9 total sessions (${allSessions}) > open tabs (${tabCount}) — the reachable set is NOT every session`
  );
  await p.close();
}
...
console.log("== DW-8.2 — split reads as ONE workspace ==");
{
  ...
  ok(
    shared.shead === 1 && shared.tabstrip === 1 && shared.composer === 1,
    `DW-8.2 shared chrome not duplicated (shead ${shared.shead}, tabstrip ${shared.tabstrip}, composer ${shared.composer} — each once)`
  );
  ok(
    shared.panes === 2,
    `DW-8.2 the split is a two-pane layout (${shared.panes} panes in one .split)`
  );
  ...
  const active = await p.evaluate(() => {
    ...
    return {
      ...
      railW: pane.querySelector(".pane-rail")
        ? getComputedStyle(pane.querySelector(".pane-rail")).width
        : "0px",
  ...
  ok(
    active.aria === "true",
    `DW-8.2 active pane carries aria-selected="true" (${active.aria})`
  );
  ok(
    active.rail && Number.parseFloat(active.railW) >= 1,
    `DW-8.2 active pane has a left rail indicator (rail width ${active.railW})`
  );
  ...
  ok(
    activeWord === "Active",
    `DW-8.2 active pane carries the visible word "Active" (${activeWord || "none"})`
  );
  ...
  ok(
    !!railColor,
    `DW-8.2 rail declares a fill (${railColor}) — but it is one of several cues`
  );
  ...
  ok(
    inact === 1,
    `DW-8.2 exactly one background pane is not marked active (${inact})`
  );
  await p.close();
}
...
console.log("== DW-8.3 — fixed anchors stable across ws states ==");
{
  ...
  for (const ws of ["split", "tabbed", "reordered"]) {
    ...
    for (const k of ["home", "input", "stop"]) {
      if (now[k] !== base[k]) {
        stable = false;
        console.log(`    anchor ${k} moved in ${ws}: ${base[k]} -> ${now[k]}`);
      }
    }
  }
  ok(
    stable,
    "DW-8.3 Home, composer input, Stop hold identical positions across split / tabbed / reordered"
  );
  ...
  for (const ws of ["tabbed", "reordered"]) {
    const lone = await p.evaluate((ws) => {
      ...
      document
        ...
        .setAttribute(
          "data-order",
          ws === "reordered" ? "reordered" : "default"
        );
      const e = document.querySelector("#pane-a-lone");
      if (!e) {
        return null;
      }
      const r = e.getBoundingClientRect();
    ...
    ok(
      lone && lone.display !== "none" && lone.w > 100 && lone.h > 100,
  ...
  // fixed anchors hold under a coarse pointer too (DW-8.3 across the mobile shell)
  {
    ...
    const mrects = async () => {
      ...
      return {
        home: await r("#ws-home"),
        input: await r("#comp textarea"),
        stop: await r("#comp .stop"),
      };
    };
    ...
    for (const ws of ["split", "tabbed", "reordered"]) {
      ...
      for (const k of ["home", "input", "stop"]) {
        if (now[k] !== mbase[k]) {
          mstable = false;
          console.log(
            `    mobile anchor ${k} moved in ${ws}: ${mbase[k]} -> ${now[k]}`
          );
        }
      }
    }
    ok(
      mstable,
      "DW-8.3 anchors stable across ws states under coarse pointer (mobile shell)"
    );
    await mp.close();
  }
  await p.close();
...
// ---------------------------------------------------------------------------
console.log(
  "== DW-8.5 — transcript pane does NOT carry touch-action: pan-y =="
);
{
  const p = await open({});
  await p.evaluate(SAMPLER);
  const ta = await p.evaluate(
    () => getComputedStyle(document.querySelector(".tr-pane")).touchAction
  );
  ok(
    !!ta && ta !== "pan-y",
    `DW-8.5 transcript pane touch-action is NOT pan-y (computed: "${ta}")`
  );
  const cssNoComments = styleText.replace(/\/\*[\s\S]*?\*\//g, "");
  ok(
    !/touch-action\s*:\s*pan-y/.test(cssNoComments),
    "DW-8.5 no touch-action: pan-y rule anywhere (the deviation is intact)"
  );
  ...
  ok(
    hasScrollDesc,
    "DW-8.5/8.8 the pane contains a genuinely sideways-scrolling code block (overflow-x:auto, content overflows)"
  );
  const paneTa = await p.evaluate(
    () => getComputedStyle(document.querySelector(".tr-pane")).touchAction
  );
  ok(
    !["pan-y", "none"].includes(paneTa),
    `DW-8.8 the pane does not claim horizontal pan (touch-action "${paneTa}"), so the descendant owns it`
  );
  await p.close();
}
...
// ---------------------------------------------------------------------------
console.log(
  "== DW-8.8 — no tab-handler on the sideways descendant; it scrolls =="
);
{
  const p = await open({});
  await p.evaluate(SAMPLER);
  const desc = await p.evaluate(() => {
    ...
    return el
      ? {
          ...
          hasTabClass: Array.from(el.querySelectorAll("*")).some((n) =>
            /tab/.test(String(n.className))
          ),
        }
      : null;
  });
  ok(
    !!desc && desc.overflowX === "auto" && desc.scrollable,
    `DW-8.8 sideways descendant genuinely scrolls horizontally (overflow-x ${desc && desc.overflowX}, scrollable ${desc && desc.scrollable})`
  );
  ok(
    desc && !desc.hasTabClass,
    "DW-8.8 the descendant contains no tab control, so a swipe inside it cannot switch the tab"
  );
  await p.close();
}
...
console.log("== DW-8.7 — every gesture has a non-gesture equivalent ==");
{
  ...
  ok(
    pg.prev && pg.next,
    "DW-8.7 the swipe pager has explicit Previous and Next buttons (non-gesture equivalents)"
  );
  ok(
    pg.close >= 4,
    `DW-8.7 every tab has a visible close button (${pg.close} of ${pg.tabs} tabs) — nothing is reachable only by swiping`
  );
  ok(
    /DRIEFLY|non-gesture equivalent/.test(raw) ||
      /EVERY GESTURE HAS A NON-GESTURE/.test(raw),
...
// ---------------------------------------------------------------------------
console.log(
  "== DW-8.4 rows 4/5/8 + coarse-pointer targets at mobile widths =="
);
...
for (const width of [390, 430, 768]) {
  ...
  ok(
    m.sw <= m.iw,
    `v5-workspace @${width}: no horizontal document overflow (scrollWidth ${m.sw} vs ${m.iw})`
  );
  ok(
    Number.parseFloat(m.inputFs) >= 16,
    `v5-workspace @${width}: composer input ${m.inputFs}px (>=16px, DW-8.4.4)`
  );
  ok(
    m.paneTa === "manipulation",
    `v5-workspace @${width}: pane touch-action manipulation present (${m.paneTa})`
  );
  ok(
    m.tapHighlight === "rgba(0, 0, 0, 0)" || m.tapHighlight === "transparent",
    `v5-workspace @${width}: tap-highlight neutralised (${m.tapHighlight})`
  );
  ok(
    m.tab && m.tab[0] >= 44 && m.tab[1] >= 44,
    `v5-workspace @${width}: tab target >=44x44 (${m.tab})`
  );
  const preFs = await p.evaluate(() =>
    Number.parseFloat(
      getComputedStyle(document.querySelector(".code-scroll pre")).fontSize
    )
  );
  ok(
    preFs >= 16,
    `v5-workspace @${width}: code-block text readable at >=16px (${preFs}px)`
  );
  await p.close();
}
// pointer-down feedback (row 5) — some :active rule must exist on controls
{
  ...
  ok(
    hasActive,
    "DW-8.4.5 interactive controls present; :active press feedback is declared in the stylesheet"
  );
  const activeRules = (
    styleText.replace(/\/\*[\s\S]*?\*\//g, "").match(/:active/g) || []
  ).length;
  ok(
    activeRules >= 2,
    `DW-8.4.5 :active (pointer-down) feedback rules present (${activeRules})`
  );
  const uSelect = (
    styleText
      .replace(/\/\*[\s\S]*?\*\//g, "")
      .match(/user-select\s*:\s*none/g) || []
  ).length;
  ok(
    uSelect >= 5,
    `DW-8.4.8 user-select:none on controls (${uSelect} declarations)`
  );
  const manip = (
    styleText
      .replace(/\/\*[\s\S]*?\*\//g, "")
      .match(/touch-action\s*:\s*manipulation/g) || []
  ).length;
