...
const FILES = [
  'v2-fleet.html', 'v3-assistant.html', 'v4-transcript.html',
  'v5-components.html', 'v5-agent.html', 'v5-data.html',
];
...
for (const f of FILES) {
  const p = joinPath(f);
  if (!existsSync(p)) { fail(`missing built mock ${f} — run build-mocks.sh first`); continue; }
  files[f] = readFileSync(p, 'utf8');
}
function joinPath(name) { return join(DIR, name); }
...
// ===========================================================================
{
  const BANNED = ['Oops', 'Are you sure', 'Invalid input', 'Click here', 'Learn more', 'Success!', 'Submit'];
  ...
  for (const b of BANNED) {
    ...
    if (cnt) { hits += cnt; console.log(`    banned "${b}" x${cnt}`); }
  }
  totalChecks += 1;
  hits === 0
  ...
  const bang = (allText.match(/[^>]![^<]/g) || []).filter((s) =>
    /error|fail|unreachable|offline|reconnect|denied|expired|disconnected|cannot|retry|hub/i.test(s));
  totalChecks += 1;
  bang.length === 0
...
// ===========================================================================
{
  ...
  const weBlam = sentences.filter((s) =>
    /\b(?:error|fail|unreachable|offline|expired|denied|reconnect|disconnected|blocked|locked)\b/i.test(s) &&
  ...
  const errSent = sentences.filter((s) =>
    /\b(?:error|fail|unreachable|offline|expired|denied|reconnect|disconnected|blocked|locked|invalid)\b/i.test(s));
  const blame2 = errSent.filter((s) => /you (?:entered|typed|provided|tried|submitted|caused|triggered)/i.test(s));
  totalChecks += 1;
  blame2.length === 0
...
// ===========================================================================
{
  const BANNED_SYNONYMS = [
    { re: /\b\d+\s+delegates?\b/i,                                             label: 'child-task count named "delegate" (should be subagent)' },
    { re: /\bdelegate\b[^\n]{0,40}\bfixture-check\b/i,                          label: 'branch tag "delegate" for an in-session subagent' },
    { re: /\bthe\s+delegate\s+branch\b/i,                                       label: '"the delegate branch" (in-session child = subagent)' },
    { re: /\bdelegat(?:ed?)\b[^<]{0,30}\b(?:to\s+an?\s+)?pi\s+subagent\b/i,      label: '"delegated ... subagent" conflation' },
  ];
  let hits = 0;
  for (const f of FILES) {
    if (!files[f]) continue;
    ...
    for (const { re, label } of BANNED_SYNONYMS) {
      for (const m of (body.match(re) || [])) {
        hits += 1; console.log(`    [${f}] ${label}: ${m.trim().slice(0, 60)}`);
      }
    }
  }
  totalChecks += 1;
...
// ===========================================================================
{
  ...
  for (const f of FILES) {
    ...
    while ((m = re.exec(html)) !== null) {
      ...
      if (/type=["'](?:checkbox|hidden|radio|submit|button)["']/.test(tag)) continue;
      if (/\bhidden\b/.test(tag)) continue;
      // demo scope: component-gallery state cells are an atlas, not a user flow
      const before = html.slice(0, m.index);
      const inCell = /<div class="inner">[\s\S]*$/.test(before)
      ...
      const hasLabelFor = !!id && new RegExp(`<label[^>]*\\bfor=["']${id}["']`).test(win);
      if (!(hasAria || hasLabelFor)) BAD.push(`${f}: ${tag.trim().slice(0, 80)}`);
    }
  }
  totalChecks += 1;
  (controls > 0 && BAD.length === 0)
    ? pass(`DW-7.7 ${controls} real form controls each have a visible label or aria-label`)
...
{
  const VERBS = new Set([
    'start', 'create', 'add', 'save', 'cancel', 'delete', 'remove', 'approve', 'allow', 'deny',
    'reconnect', 'retry', 'export', 'filter', 'manage', 'open', 'stop', 'interrupt', 'send',
    'attach', 'refresh', 'install', 'fetch', 'restore', 'redirect', 'skip', 'continue', 'view',
    'always', 'connect', 'new', 'edit', 'move', 'copy', 'search', 'reload', 'reset', 'peek',
    'pause', 'resume', 'collapse', 'expand', 'favourite', 'toggle', 'review', 'revoke', 'leave',
    'finish', 'resolve', 'show', 'hide', 'apply', 'set', 'select', 'delete', 'forget', 'publish',
    'deploy', 'revert', 'undo', 'sign', 'login', 'log', 'answer', 'send', 'type', 'clear',
  ]);
  ...
  for (const f of FILES) {
    ...
    while ((m = re.exec(html)) !== null) {
      ...
      if (/tabindex="-1"/.test(open)) continue;
      if (/class="[^"]*\b(?:sel|pgbtn|page|pager|dense|x|ic|pill|dot|ruler|tile|cfg|home|back|burger|stop|qfree)[^"]*"/.test(open)) continue;
      if (/class="[^"]*\bst-(?:error|success|loading|hover|focus|press|disabled)\b/.test(open)) continue;
      ...
      if (/^[A-Z][a-z]+(\s+(?:the|this|sidebar|board|theme|token|dark|session)){0,2}$/.test(txt)
          && /aria-label=/.test(open)) continue;
      // single-word state/toggle values and answer options are not action CTAs
      if (/^(light|dark|comfortable|compact|[0-9]+|all\s+\w+|loading…|syncing…|starting…|connecting…)$/i.test(txt)) continue;
      // numeric-prefixed answer options (question cards)
      if (/^[0-9]+\s+[A-Z]/.test(txt)) continue;
      // a leading bare number is not a verb (keep verb check honest); treat as answer/option
      checked += 1;
      if (BANNED_LABEL.test(txt)) { BAD.push(`${f}: banned generic "${txt}"`); continue; }
      if (/\b(?:submit|ok|yes|click here|learn more|confirm)\b/i.test(txt) && !/^[A-Z]/.test(txt)) {
        BAD.push(`${f}: banned bare generic "${txt}"`); continue;
      }
      const first = (txt.split(/\s+/)[0] || '').replace(/[^\w]/g, '').toLowerCase();
      if (first && !VERBS.has(first) && !/[{]/.test(txt) && !/^\d/.test(txt)) {
        if (!/\?$/.test(txt) && !/^(failed|saved|done|error|recorded|rejected|active|connected|paused)$/i.test(txt)) {
          BAD.push(`${f}: "${txt}" (first word "${first}" not an allowlisted verb)`);
        }
      }
    }
  }
...
// ===========================================================================
{
  const FIX = /(?:reconnect|retry|check|handoff:|try\s+again|restart|sign\s+in|pick\s+another|unlock|redirect|re-(?:plans?|platforms?)|start|re-|review|resolve|refresh|wait|run|add|install|remove|upgrade|update)/i;
  const BAD = [];
  for (const f of FILES) {
    if (!files[f]) continue;
    const html = files[f]
    ...
    const re = /<(?:div|p|span|strong)[^>]*class="[^"]*(?:failcard|attncard|handoff|band)[^"]*"[^>]*>([\s\S]*?)<\/(?:div|p|span|strong)>/gi;
    let m;
    while ((m = re.exec(html)) !== null) {
      const inner = m[1].replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
      if (!inner) continue;
      if (!FIX.test(inner)) BAD.push(`${f}: "${inner.slice(0, 70)}"`);
    }
  }
  totalChecks += 1;
  BAD.length === 0
...
// ===========================================================================
console.log('\nwordcheck summary:');
console.log(`  check groups: 7  ·  assertions: ${totalChecks}  ·  failures: ${fails}`);
if (fails === 0) {
