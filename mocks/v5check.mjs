...
async function open({ scheme = 'light', compact = false, width = 1440, height = 5200, coarse = false } = {}) {
  const ctx = await browser.newContext({
    viewport: { width, height }, deviceScaleFactor: 2,
    hasTouch: coarse, isMobile: coarse,
  });
  const page = await ctx.newPage();
  await page.goto(pathToFileURL(join(HERE, 'v5-components.html')).href, { waitUntil: 'load' });
...
for (const scheme of ['light', 'dark']) {
  ...
  for (const [name, sel] of Object.entries(probes)) {
    ...
    if (!r) { ok(false, `${scheme}: ${name} NOT FOUND`); continue; }
    ok(r.w > 0 && r.h > 0, `${scheme}: ${name} sized ${r.w}x${r.h}`);
  }
  // destructive modal: second .modal present
  const nmod = await p.evaluate(() => document.querySelectorAll('.modal').length);
  ok(nmod === 2, `${scheme}: two modals present (got ${nmod})`);
  // idle pill has no fill
  const idleBg = await p.evaluate((s) => window.__rgb(s, 'background-color'), '.pill.idle');
  ok(idleBg === null, `${scheme}: idle pill has NO fill`);
  // scrim paints
  const scrim = await p.evaluate((s) => window.__rgb(s, 'background-color'), '.scrim.bare');
  ok(!!scrim || scrim === null && false, `${scheme}: scrim bg present to evaluate`);
  // modal ~460px
  const mw = await p.evaluate(() => document.querySelector('.modal').getBoundingClientRect().width);
  ...
  const panel = await p.evaluate((s) => window.__rgb(s, 'background-color'), '.panel');
  const well = await p.evaluate((s) => window.__rgb(s, 'background-color'), '.panel .well');
  ok(JSON.stringify(panel) !== JSON.stringify(well) && !!panel && !!well,
    `${scheme}: raised panel ${panel} differs from well ${well} (inset elevation)`);
  await p.close();
}

// =====================================================================
console.log('== density = dimension-only: one tier, colours identical, sizes smaller ==');
{
  const cols = ['color', 'background-color', 'background-image', 'border-top-color'];
  ...
  for (const compact of [false, true]) {
    ...
    const snap = await p.evaluate(() => {
      const grab = (s, prop) => getComputedStyle(document.querySelector(s)).getPropertyValue(prop).trim();
      return {
        btnH: grab('.btn.pri', 'height'), btnPad: grab('.btn.pri', 'padding-left'),
        btnFill: grab('.btn.pri', 'background-color'), btnGrad: grab('.btn.pri', 'background-image'),
        inpFs: grab('.inp input', 'font-size'), navH: grab('.rail .nav-i', 'height'),
        pillFs: grab('.pill.live', 'font-size'), pageH: grab('.page.on', 'height'),
        wellBg: grab('.panel .well', 'background-color'),
        liveBg: grab('.pill.live', 'background-color'), liveInk: grab('.pill.live', 'color'),
        ringOut: grab('.st-focus', 'outline-width') + ' ' + grab('.st-focus', 'outline-style') + ' ' + grab('.st-focus', 'outline-color'),
  ...
  const comfy = gDatasets.comfy, compact = gDatasets.compact;
  for (const k of ['btnFill', 'btnGrad', 'wellBg', 'liveBg', 'liveInk', 'ringOut', 'ringOff', 'modalW']) {
    ok(comfy[k] === compact[k], `density keeps colour/ring/structural-width ${k} identical (${comfy[k]} vs ${compact[k]})`);
  }
  ok(parseFloat(comfy.btnH) > parseFloat(compact.btnH), `button height ${comfy.btnH} -> ${compact.btnH}`);
  ok(parseFloat(comfy.navH) > parseFloat(compact.navH), `nav height ${comfy.navH} -> ${compact.navH}`);
  ok(parseFloat(comfy.pageH) > parseFloat(compact.pageH), `page button ${comfy.pageH} -> ${compact.pageH}`);
  ok(parseFloat(comfy.inpFs) >= 16 && parseFloat(compact.inpFs) >= 16,
...
for (const scheme of ['light', 'dark']) {
  const p = await open({ scheme });
  await p.evaluate(SAMPLER);
  const ring = await p.evaluate((s) => window.__cs(s, 'outline-width') + ' ' + window.__cs(s, 'outline-style') + ' ' + window.__cs(s, 'outline-color'), '.st-focus');
  const off = await p.evaluate((s) => window.__cs(s, 'outline-offset'), '.st-focus');
  ok(ring.startsWith('2px solid'), `${scheme}: focus ring ${ring}`);
  ok(!ring.endsWith('transparent') && ring.includes('rgb'), `${scheme}: ring colour visible`);
  ok(off === '2px', `${scheme}: ring offset ${off}px`);
  await p.close();
}

// =====================================================================
console.log('== responsive: 320/390/430 no overflow; coarse targets >=44px; input 16px ==');
for (const scheme of ['light', 'dark']) {
...
console.log('== painted contrast: status pills + destructive, both schemes ==');
{
  ...
  for (const scheme of ['light', 'dark']) {
    ...
    for (const [sel, , name] of pairs) {
      ...
      const bg = await p.evaluate((s) => window.__rgb(s, 'background-color'), sel);
      if (!ink || !bg) { ok(false, `${scheme}: ${name} — could not sample ink ${ink} bg ${bg}`); continue; }
      const r = ratio(ink, bg);
      // radio.text on a tinted ghost uses the panel field; treat bg on its own fill
      ok(r >= 4.5, `${scheme}: ${name} ${r.toFixed(2)}:1 (ink ${ink} on ${bg})`);
    }
    await p.close();
  }
}

console.log('== density: compact rects strictly smaller than comfortable (pixel gate input) ==');

const RECTS = ['navOn', 'btnPri', 'btnDes', 'btnDesSol', 'btnIcon', 'pillLive', 'pillAttn',
               'pillFail', 'pillIdle', 'panel', 'well', 'inp', 'sel', 'pageOn',
               'modalScrim', 'modalFt', 'focRing', 'mark1', 'mark2', 'mark3', 'sheet', 'close'];
const RECT_SEL = {
  navOn: ['.rail .nav-i.on', 0], btnPri: ['.btn.pri', 0], btnDes: ['.btn.des', 0],
  btnDesSol: ['.btn.des.sol', 0], btnIcon: ['.btn.ic', 0],
  pillLive: ['.pill.live', 0], pillAttn: ['.pill.attn', 0], pillFail: ['.pill.fail', 0],
  pillIdle: ['.pill.idle', 0], panel: ['.panel', 0], well: ['.panel .well', 0],
  inp: ['.inp', 0], sel: ['.sel', 0], pageOn: ['.page.on', 0],
  modalScrim: ['.scrim.bare', 0], modalFt: ['.mod-ft', 0], focRing: ['.st-focus', 0],
  mark1: ['.mark-row .mark', 0], mark2: ['.mark-row .mark', 1], mark3: ['.mark-row .mark', 2],
  sheet: ['.sheet', 0], close: ['.mod-hd .close', 0],
};
{
  const fs = await import('node:fs');
  for (const [label, scheme, compact] of [['light', 'light', false], ['dark', 'dark', false], ['compact', 'light', true]]) {
    const p = await open({ scheme, compact });
    const out = await p.evaluate((sels) => {
      const o = {};
      for (const [name, [sel, i]] of Object.entries(sels)) {
        const el = document.querySelectorAll(sel)[i];
        if (!el) { o[name] = null; continue; }
        const r = el.getBoundingClientRect();
        o[name] = [Math.round(r.x), Math.round(r.y), Math.round(r.width), Math.round(r.height)];
      }
      return o;
    }, RECT_SEL);
  ...
  console.log('  written rects-light.json / rects-dark.json / rects-compact.json');
}
