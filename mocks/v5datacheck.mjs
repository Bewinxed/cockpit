...
async function open({
  ...
  await page.goto(pathToFileURL(join(HERE, "v5-data.html")).href, {
    waitUntil: "load",
  });
  if (scheme === "dark") {
...
console.log("== DW-6.1: bar value axis starts at ZERO ==");
{
  ...
  const m = await p.evaluate(() => {
    ...
    const axisY = Number.parseFloat(line.getAttribute("y1"));
    ...
    const rects = Array.from(svg.querySelectorAll("rect")).map((r) => ({
      x: Math.round(Number.parseFloat(r.getAttribute("x"))),
    ...
    const dayBottoms = [...byDay.values()].map((stack) =>
      Math.max(...stack.map((s) => s.y + s.h))
    );
    ...
    return {
      ...
      dayBottoms,
      nDays: byDay.size,
      hasZero: labels.includes("0"),
      nSeries,
    };
  });
  ok(
    m.rects.length >= 21,
    `DW-6.1 chart has ${m.rects.length} bar segments (>=21)`
  );
  ok(m.nDays === 7, `DW-6.1 chart has ${m.nDays} day stacks (7)`);
  ok(m.hasZero, 'DW-6.1 value axis carries the "0" label');
  ok(
    m.dayBottoms.every((b) => b === m.axisY),
  ...
  ok(
    m.rects.every((r) => r.y >= 0),
    "DW-6.1 no bar segment extends below the baseline (no negative value)"
  );
  await p.close();
}
...
// =====================================================================
console.log(
  "== DW-6.2 / DW-6.10: stat cards carry baseline + direction + favorability; <=7, visible =="
);
{
  const p = await open({});
  ...
  const dirs = await p.evaluate(() =>
    Array.from(document.querySelectorAll(".stats .stat")).map(
      (c) =>
        c.querySelector(".dir")?.classList.contains("up") ||
        c.querySelector(".dir")?.classList.contains("dn")
    )
  );
...
console.log("== DW-6.3: zero gauges; meters are bullet-shaped ==");
{
  ...
  const gauge = await p.evaluate(() => {
    ...
    for (const el of Array.from(document.querySelectorAll("*"))) {
      ...
      if (/(conic|radial)-gradient/.test(bg)) {
        bad.push(`${el.tagName}.${el.className}:${bg.slice(0, 30)}`);
      }
    }
    return bad;
  });
  ok(
    gauge.length === 0,
    `DW-6.3 zero gauge/radial meters (${gauge.length} conic/radial gradients)`
  );
  const bullets = await p.evaluate(() => {
    ...
    return els.map((e) => {
      ...
      return {
        n: e.querySelector(".cap .nm")?.textContent,
  ...
  ok(
    bullets.length === 2,
    `DW-6.3 two bullet meters present (${bullets.length})`
  );
  for (const b of bullets) {
    ok(
      b.w > b.h * 4,
      `DW-6.3 "${b.n}" bullet is a wide bar (${b.w}x${b.h}px), not a circle`
    );
    ok(
      b.hasBar && b.hasTarget && b.hasRange,
...
// =====================================================================
console.log(
  "== DW-6.4 / DW-6.5: threshold ink is status, discrete, never colour-alone =="
);
for (const scheme of ["light", "dark"]) {
  ...
  const thr = await p.evaluate(() => {
    ...
    const okay = grab(".thr.ok"),
      warn = grab(".thr.warn"),
      bad = grab(".thr.bad");
    const sol = {};
    ["--data-ok", "--data-warn", "--data-bad"].forEach((t) => {
      sol[t] = getComputedStyle(document.documentElement)
        .getPropertyValue(t)
        .trim();
    });
    const cells = Array.from(document.querySelectorAll(".thr")).map((c) => ({
      ...
      hasWord:
        !!c.querySelector(".lvl") &&
        c.querySelector(".lvl").textContent.trim().length > 0,
    }));
  ...
  ok(
    missingCue === 0,
    `${scheme}: DW-6.4 every threshold value has icon + word (${missingCue} missing)`
  );
  // DW-6.5: the three levels are DISCRETE (distinct colours), never interpolated
  const set = new Set([thr.okay, thr.warn, thr.bad]);
  ok(
    set.size === 3,
    `${scheme}: DW-6.5 exactly 3 discrete threshold colours (${set.size})`
  );
...
// =====================================================================
console.log(
  "== DW-6.6: status hues only on status indicators; chart uses the categorical palette =="
);
{
  const p = await open({});
  await p.evaluate(SAMPLER);
  const palette = await p.evaluate(() => {
    ...
    grid.style.cssText = "position:absolute;left:-9999px;top:0";
    const mk = (v, id) => {
    ...
    mk(ret.dataOk, "sdataOk");
    mk(ret.dataWarn, "sdataWarn");
    mk(ret.dataBad, "sdataBad");
    mk(cs.getPropertyValue("--error-9"), "serr9");
    mk(cs.getPropertyValue("--warning-9"), "swarn9");
    ...
    ret.asp = {
      dataOk: rgb("sdataOk"),
      dataWarn: rgb("sdataWarn"),
      dataBad: rgb("sdataBad"),
      err9: rgb("serr9"),
      warn9: rgb("swarn9"),
      suc9: rgb("ssuc9"),
    };
    // chart series fills
    ret.series = Array.from(
      document.querySelectorAll("svg.chart-box rect")
    ).map((r) => r.getAttribute("fill"));
    ret.statusFills = Array.from(document.querySelectorAll(".pill,.fav")).map(
  ...
  const seriesColors = await p.evaluate(() =>
    Array.from(document.querySelectorAll("svg.chart-box rect")).map((r) => {
      ...
      ctx.fillRect(0, 0, 1, 1);
      const dat = ctx.getImageData(0, 0, 1, 1).data;
      d.remove();
...
console.log("== DW-6.7: numeric right, text left, no full cell borders ==");
{
  ...
  const tbl = await p.evaluate(() => {
    ...
    const allTdTxt = tds
      .filter(
        (t) => !(t.classList.contains("num") || t.classList.contains("mono"))
      )
      .every((t) => isLeft(r(t)));
  ...
  ok(
    tbl.allThNum && tbl.allTdNum,
    "DW-6.7 numeric columns right-aligned (header + cells)"
  );
  ok(
    tbl.allThTxt && tbl.allTdTxt,
    "DW-6.7 text columns left-aligned (header + cells)"
  );
  ok(
    tbl.borderViolations === 0,
...
// =====================================================================
console.log(
  "== DW-6.8: chart alt/aria-label pattern + aria-describedby data table =="
);
{
  const p = await open({});
  ...
  ok(
    ch.role === "img",
    `DW-6.8 chart is role=img (presentational? ${ch.role})`
  );
  ok(
    /^[^:]+ showing /i.test(ch.l || ""),
  ...
  ok(
    /^[^:]+ showing [^,]+, [^.]+\. .+/.test(ch.l || ""),
    "DW-6.8 aria-label has the [subject], [period]. [insight] shape"
  );
  ok(
    ch.d && ch.exists,
    `DW-6.8 aria-describedby present and resolves (${ch.d})`
  );
  await p.close();
}
...
// =====================================================================
console.log(
  "== B1 (Never #3): delta arrows are neutral ink, not status/data hues =="
);
{
  const p = await open({});
  await p.evaluate(SAMPLER);
  const m = await p.evaluate(() => {
    ...
    const arrows = Array.from(document.querySelectorAll(".stat .d .dir")).map(
      (a) => ({
        cls: a.className,
    ...
    const arrowRgb = (c) => {
      ...
      cv.width = cv.height = 1;
      const ctx = cv.getContext("2d", { willReadFrequently: true });
      ctx.fillStyle = getComputedStyle(d).color;
      ctx.fillRect(0, 0, 1, 1);
      const dat = ctx.getImageData(0, 0, 1, 1).data;
      d.remove();
      return [dat[0], dat[1], dat[2]].join(",");
  ...
  for (const a of m.arrows) {
    if (m.forbidden.includes(a.rgb)) {
      clashes++;
      badCls.push(`${a.cls}=${a.rgb}`);
    }
  }
  ok(
    clashes === 0,
    `B1 zero delta arrows painted in --data-ok/--data-bad/--error-9/--success-9 (${badCls.join(", ") || "all neutral"})`
  );
  const allInk = m.arrows.every((a) => a.rgb === m.inkBody);
  ok(
    allInk,
    `B1 every delta arrow renders the neutral ink-body (${m.inkBody}) — Cairo: encode direction by glyph/sign, never by a lying hue`
  );
  await p.close();
}
...
// =====================================================================
console.log(
  "== B2 (DW-6.5): every threshold cell classifies its value into the named band =="
);
for (const scheme of ["light", "dark"]) {
  const p = await open({ scheme });
  await p.evaluate(SAMPLER);
  const cells = await p.evaluate(() =>
    Array.from(document.querySelectorAll(".thr")).map((c) => ({
      cls: c.className,
      val: Number.parseFloat((c.textContent.match(/([\d.]+)%/) || [])[1]),
  ...
  for (const c of cells) {
    if (Number.isNaN(c.val)) {
      wrongBand++;
      bad.push(`${c.cls} val unparseable`);
      continue;
    }
    ...
    if (c.lvl !== word[exp]) {
      wrongWord++;
      bad.push(`${c.val}% -> word "${c.lvl}" should be "${word[exp]}"`);
    }
  }
  ok(
    wrongBand === 0,
    `${scheme}: B2 every .thr class matches its value band (ok<70, warn 70–90, bad>90) — ${bad.join("; ") || "all correct"}`
  );
  ok(
    wrongWord === 0,
    `${scheme}: B2 every .thr breakpoint word matches its band (${bad.length ? "" : "ok/moving/critical consistent"})`
  );
  ok(
    cells.every((c) => c.icon),
    `${scheme}: B2 every .thr value still carries its non-colour icon (DW-6.4)`
  );
  await p.close();
}
...
// =====================================================================
console.log(
  "== B3 (Nielsen #3/#6): coarse <=900px has an opener, >=44x44, toggles the drawer on-screen =="
);
{
  ...
  ok(
    opener.r[0] >= 44 && opener.r[1] >= 44,
    `B3 opener is ${opener.r[0]}x${opener.r[1]}px (coarse target >=44x44)`
  );
  ok(
    opener.tag === "BUTTON" || opener.tag === "A",
    `B3 opener is a real focusable control (${opener.tag}.${opener.cls})`
  );
  ok(
    !!opener.labelled,
    `B3 opener has an accessible name (${opener.labelled})`
  );
  ok(
    opener.controls && opener.controlsExists,
    `B3 opener aria-controls resolves to the drawer (${opener.controls})`
  );
  ok(
    opener.expanded !== null,
    `B3 opener tracks aria-expanded (${opener.expanded})`
  );
  ...
  ok(
    toggled.left >= 0,
    `B3 toggling .nav-open brings the aside on-screen (left ${toggled.left}px >= 0) — Fleet/Tools/Rules/Usage reachable`
  );
  ok(
    toggled.opacity === "1",
    `B3 scrim is active over the board while the drawer is open (opacity ${toggled.opacity})`
  );
...
// =====================================================================
console.log(
  "== B4 (DW-6.8): EVERY chart carries role=img + [type] showing [subject], [period]. [insight] + resolvable describedby =="
);
{
  const p = await open({});
...
// =====================================================================
console.log(
  "== DW-6.9: Okabe-Ito categorical palette, CVD-survivable redundant encoding =="
);
{
  const p = await open({});
  await p.evaluate(SAMPLER);
  ...
  for (let i = 0; i < okh.length; i++) {
    for (let j = i + 1; j < okh.length; j++) {
  ...
  ok(
    minPair > 0.05,
    `DW-6.9 series pairwise OKLab-separated (min dE ${minPair.toFixed(3)} > 0.05 JND-margin) — no two series collapse under deutan/protan`
  );
  await p.close();
}
...
// =====================================================================
console.log(
  "== density = dimension-only (DW-4.2 carry-over): colours identical, sizes differ =="
);
{
  const grab = async (compact) => {
    ...
    const s = await p.evaluate(() => ({
      ...
      pillBg: getComputedStyle(document.querySelector(".pill.live"))
        .backgroundColor,
      dataWarn: getComputedStyle(document.documentElement)
        .getPropertyValue("--data-warn")
        .trim(),
  ...
  for (const k of ["pillBg", "dataWarn", "font"]) {
    ok(
      comfy[k] === compact[k],
      `density keeps ${k} identical (${comfy[k]} vs ${compact[k]})`
    );
  }
  ok(
    Number.parseFloat(comfy.rowH) > Number.parseFloat(compact.rowH),
    `table row ${comfy.rowH} -> ${compact.rowH}`
  );
  ok(
    Number.parseFloat(comfy.statH) > Number.parseFloat(compact.statH),
    `stat card ${comfy.statH} -> ${compact.statH}`
  );
}

// =====================================================================
console.log(
  "== responsive: 320/390/430 no overflow on coarse pointer; bullets/table fit =="
);
for (const width of [320, 390, 430]) {
  ...
  const m = await p.evaluate(() => ({
    ...
    rowH: getComputedStyle(document.querySelector(".t td")).height,
    pg: (() => {
      const r = document.querySelector(".pgbtn").getBoundingClientRect();
      return [Math.round(r.width), Math.round(r.height)];
    })(),
    btn: (() => {
      const r = document.querySelector(".phead .btn").getBoundingClientRect();
      return [Math.round(r.width), Math.round(r.height)];
    })(),
  }));
  ok(
    m.sw <= m.iw,
    `v5-data @${width}: no horizontal document overflow (scrollWidth ${m.sw} vs ${m.iw})`
  );
  ok(
    Number.parseFloat(m.rowH) >= 44,
    `v5-data @${width}: table row ${m.rowH}px under coarse (>=44)`
  );
  ok(
    m.pg[0] >= 44 && m.pg[1] >= 44,
    `v5-data @${width}: pagination target ${m.pg[0]}x${m.pg[1]}`
  );
  ok(
    m.btn[0] >= 44 && m.btn[1] >= 44,
    `v5-data @${width}: header button ${m.btn[0]}x${m.btn[1]}`
  );
  await p.close();
}
