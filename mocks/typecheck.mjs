...
const STEPS = new Set(['10.25px', '11.5px', '13px', '14.5px', '16.5px',
  '18.5px', '20.75px', '23.5px', '26.25px']);
...
let fails = 0;
const weights = {}, sizes = {}, normals = [], decorated = [];
    ...
    const r = await page.evaluate(() => {
      ...
      for (const el of document.querySelectorAll('body *')) {
        ...
        if (cs.lineHeight === 'normal') {
          n.push((el.tagName + '.' + String(el.className || '')).slice(0, 40));
        }
        ...
        if (cs.textDecorationLine && cs.textDecorationLine !== 'none') {
          u.push((el.tagName + '.' + String(el.className || '')).slice(0, 40)
            + ' ' + cs.textDecorationLine);
        }
      }
      return { w, s, n, u };
    });
    for (const [k, v] of Object.entries(r.w)) weights[k] = (weights[k] || 0) + v;
    for (const [k, v] of Object.entries(r.s)) sizes[k] = (sizes[k] || 0) + v;
    for (const x of r.n) normals.push(`${file}${dark ? ' dark' : ''} ${x}`);
    for (const x of r.u) decorated.push(`${file}${dark ? ' dark' : ''} ${x}`);
    await page.close();
  }
  }
}
...
if (badW.length) { fails++; console.log(`    FAIL weights outside the 400/450/500 ladder: ${badW.join(', ')}`); }

console.log('  computed font-sizes  :', JSON.stringify(sizes));
const badS = Object.keys(sizes).filter((k) => !STEPS.has(k));
if (badS.length) { fails++; console.log(`    FAIL sizes not on the enumerated ladder: ${badS.join(', ')}`); }

if (normals.length) {
  fails++;
  console.log(`    FAIL ${normals.length} text-bearing element(s) left on line-height: normal`);
  for (const x of [...new Set(normals)].slice(0, 8)) console.log(`      ${x}`);
} else {
  console.log('  line-height: normal  : none — every text element has a specified line box');
}

if (decorated.length) {
  fails++;
  console.log(`    FAIL ${decorated.length} element(s) carry a UA text-decoration`);
  for (const x of [...new Set(decorated)].slice(0, 6)) console.log(`      ${x}`);
} else {
  console.log('  text-decoration      : none anywhere — no leaked UA underline');
}
...
for (const width of PAIR_WIDTHS) {
  ...
  for (const dark of [false, true]) {
    ...
    await page.goto(pathToFileURL(join(HERE, 'v4-transcript.html')).href, { waitUntil: 'load' });
    if (dark) await page.evaluate(() => document.documentElement.classList.add('dark'));
    await page.evaluate(() => document.fonts.ready);
    const pair = await page.evaluate(() => [...document.querySelectorAll('.choice button')].map((el) => {
      const cs = getComputedStyle(el); const r = el.getBoundingClientRect();
      return { text: el.textContent.trim(), w: Math.round(r.width), h: Math.round(r.height),
        bg: cs.backgroundColor, bgi: cs.backgroundImage === 'none' ? 'none' : 'gradient',
        shadow: cs.boxShadow === 'none' ? 'none' : 'shadow', color: cs.color };
    }));
    await page.close();
    const tag = `@${width}${coarse ? ' coarse' : ''}${dark ? ' dark' : ''}`;
    if (pair.length !== 2) { fails++; console.log(`    FAIL ${tag} expected 2 actions, found ${pair.length}`); continue; }
    const [a, b] = pair;
    const line = `  ${tag.padEnd(11)} ${a.text} ${a.w}x${a.h} (${a.bgi}/${a.shadow})`
      + `  vs  ${b.text} ${b.w}x${b.h} (${b.bgi}/${b.shadow})`;
    const problems = [];
    if (a.w !== b.w || a.h !== b.h) problems.push(`size parity lost: ${a.w}x${a.h} vs ${b.w}x${b.h}`);
    if (a.bg === b.bg && a.bgi === b.bgi && a.color === b.color && a.shadow === b.shadow) {
      problems.push('visually interchangeable');
    }
    const flat = pair.filter((x) => x.bgi === 'none' && x.shadow === 'none');
    if (flat.length) problems.push(`${flat.map((x) => x.text).join(', ')} is flat`);
    if (problems.length) { fails += problems.length; console.log(`    FAIL ${line}\n         ${problems.join('; ')}`); }
...
console.log(fails ? `  ${fails} type/action conformance failures` : '  render matches the locked type system and the action pair is distinct');
