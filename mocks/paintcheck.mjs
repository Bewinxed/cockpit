...
let fails = 0;
for (const file of ['v2-fleet.html', 'v3-assistant.html', 'v4-transcript.html']) {
  ...
  for (const dark of [false, true]) {
    const page = await browser.newPage({
      viewport: { width, height: 1023 }, deviceScaleFactor: 2,
    });
    ...
    const rects = await page.evaluate((sel) => {
      ...
      const seeThrough = (el) => {
        ...
        if (parseFloat(cs.opacity) <= 0.3) return true;
        if (cs.backdropFilter && cs.backdropFilter !== 'none') return true;
        const m = cs.backgroundColor.match(/[\d.]+\)$/);
        const alpha = m ? parseFloat(m[0]) : (cs.backgroundColor === 'transparent' ? 0 : 1);
      ...
      const textish = (el) => {
        ...
        if ((tag === 'input' || tag === 'textarea') && el.placeholder) return 'placeholder';
        return null;
      };
      ...
      return [...document.querySelectorAll(sel)].map((el) => {
        ...
        for (let a = el; a && a !== document.documentElement; a = a.parentElement) {
          ...
          if (ar.width < 1 || ar.height < 1) return null;
        }
        if (eff < 0.15) return null;
        ...
        const probes = [
          ...
          [r.x + 2, r.y + 2], [r.right - 2, r.y + 2],
          [r.x + 2, r.bottom - 2], [r.right - 2, r.bottom - 2],
        ];
        let visible = true, blocker = null;
        for (const [px, py] of probes) {
          ...
          if (idx >= 0) {
            ...
            if (!ok) blocker = blocker || stack.slice(0, idx).find((n) => !seeThrough(n));
          } else if (stack.length) {
            const anc = stack.findIndex((n) => n.contains(el));
            if (anc >= 0) {
              ok = stack.slice(0, anc).every(seeThrough);
              if (!ok) blocker = blocker || stack.slice(0, anc).find((n) => !seeThrough(n));
            } else {
        ...
        for (const n of el.childNodes) {
          ...
          for (const tr of rg.getClientRects()) {
            if (tr.width > 1 && tr.height > 1) {
              textRects.push({ x: tr.x, y: tr.y, w: tr.width, h: tr.height });
            }
          }
          rg.detach?.();
        }
        return {
          textRects,
          label: ((el.className && String(el.className)) || el.tagName)
            + (kind === 'placeholder' ? ' [placeholder]' : '')
            + ' "' + (kind === 'placeholder' ? el.placeholder : el.textContent).trim().slice(0, 14) + '"',
          x: r.x, y: r.y, w: r.width, h: r.height, visible,
          blocker: blocker ? (blocker.tagName + '.' + String(blocker.className || '')).slice(0, 30) : null,
        };
      }).filter(Boolean);
    }, TARGETS)
    ...
    VIEW_W = width;
    const inFrame = (r) => r.y >= 0 && r.y + r.h <= 1023
      && r.x >= 0 && r.x + r.w <= VIEW_W;
    const visible = rects.filter((r) => r.visible && r.w >= 8 && r.h >= 8 && inFrame(r));
    const dropped = rects.filter((r) => !(r.visible && r.w >= 8 && r.h >= 8 && inFrame(r)));
    ...
    const shot = (await page.screenshot({ animations: 'disabled', caret: 'hide' })).toString('base64');
    const measured = await page.evaluate(async ({ b64, rects }) => {
      const img = new Image();
      ...
      for (const r of rects) {
        const d = ctx.getImageData(Math.round(r.x * 2), Math.round(r.y * 2),
          Math.max(1, Math.round(r.w * 2)), Math.max(1, Math.round(r.h * 2))).data;
        const counts = new Map(); const lums = [];
        for (let i = 0; i < d.length; i += 4) {
          const k = `${d[i]},${d[i + 1]},${d[i + 2]}`;
          counts.set(k, (counts.get(k) || 0) + 1);
          lums.push({ l: d[i] * 0.299 + d[i + 1] * 0.587 + d[i + 2] * 0.114, c: [d[i], d[i + 1], d[i + 2]] });
        }
        if (!lums.length) continue;
        const top = [...counts.entries()].sort((a, b) => b[1] - a[1])[0];
        const fillShare = top[1] / (d.length / 4);
        if (fillShare < 0.4) { out.push({ label: r.label, mixed: true }); continue; }
        ...
        if (r.textRects && r.textRects.length) {
          for (const tr of r.textRects) {
            const tx = Math.max(0, Math.round(tr.x * 2)), ty = Math.max(0, Math.round(tr.y * 2));
            const tw = Math.max(1, Math.round(tr.w * 2)), th = Math.max(1, Math.round(tr.h * 2));
            if (tx + tw > cv.width || ty + th > cv.height) continue;
            const td = ctx.getImageData(tx, ty, tw, th).data;
            for (let i = 0; i < td.length; i += 4) {
              inkPool.push({ l: td[i] * 0.299 + td[i + 1] * 0.587 + td[i + 2] * 0.114,
                             c: [td[i], td[i + 1], td[i + 2]] });
            }
          }
        }
        if (!inkPool.length) inkPool = lums;
        const differing = inkPool.filter((x) => Math.abs(x.l - fillLum) > 8);
        if (differing.length < 4) { out.push({ label: r.label, noInk: true }); continue; }
        differing.sort((a, b) => a.l - b.l);
        const q = (arr, f) => arr[Math.min(arr.length - 1, Math.floor(arr.length * f))].c;
        const ink = q(differing, 0.10);
        const light = q(differing, 0.90);
    ...
    for (const d of dropped) {
      const why = !d.visible ? `occluded by ${d.blocker || 'unknown'}`
        : (d.w < 8 || d.h < 8) ? 'smaller than 8px'
        : 'rect outside the captured frame';
      console.log(`      SKIP  ${d.label.padEnd(34)} `
        + `rect ${Math.round(d.x)},${Math.round(d.y)} ${Math.round(d.w)}x${Math.round(d.h)}  — ${why}`);
    }
    for (const m of measured) {
      if (m.mixed) { mixedCount++; continue; }
      if (m.noInk) { noInkList.push(m.label); continue; }
      ...
      const ok = r >= MIN;
      if (!ok) fails++;
      console.log(`    ${ok ? 'PASS' : 'FAIL'}  ${r.toFixed(2)}:1  ${m.label.padEnd(34)}`
        + ` text rgb(${text}) on fill rgb(${m.fill})`);
    }
    if (mixedCount) console.log(`      (${mixedCount} regions span multiple fills — no single background to judge)`);
    if (noInkList.length) {
      console.log(`      NO INK  ${noInkList.length} region(s) paint nothing distinguishable from their surface:`);
      for (const l of [...new Set(noInkList)]) console.log(`        ${l}`);
    }
    await page.close();
  }
  }
