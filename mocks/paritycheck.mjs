...
// as a divergence.
const SHARED = ['.nav-i:not(.on)', '.run-i:not(.on)', '.mark', '.chip-s',
                '.act button', '.pg button', '.sec'];
...
for (const file of FILES) {
  const page = await browser.newPage({ viewport: { width: 1440, height: 1023 }, deviceScaleFactor: 1 });
  ...
  seen[file] = await page.evaluate((sels) => {
    ...
    for (const sel of sels) {
      ...
      out[sel] = {
        ...
        skeleton: [...el.children].map((c) => c.tagName.toLowerCase()).join('>'),
...
for (const sel of SHARED) {
  ...
  if (present.length < 2) {
    console.log(`  --    ${sel.padEnd(14)} present in ${present.length} file(s) — nothing to compare`);
    continue;
  }
  ...
  const detail = present.map((f) => `${f.split('-')[0]}:${seen[f][sel].icons}icon/${seen[f][sel].fill}`).join('  ');

  const iconOk = icons.size === 1;
  const fillOk = fills.size === 1;
  if (!iconOk) fails++;
  if (!fillOk) fails++;
  console.log(`  ${iconOk && fillOk ? 'PASS' : 'FAIL'}  ${sel.padEnd(14)} ${detail}`);
  if (!iconOk) console.log(`          icon presence differs between files`);
  if (!fillOk) console.log(`          resolved fill differs between files`);
  if (skels.size > 1) {
    console.log(`          note: child skeletons differ (${[...skels].join(' | ')}) — `
      + `reported, not failed; some divergence is real content`);
  }
}
console.log(fails ? `  ${fails} shared-class divergences`
