...
- Empty: two distinct readings. *First-use* — no machine has ever joined; names one next action (join a machine) and offers nothing else, because on a fresh install every surface is empty at once and this is the only one that can start the operator moving. *User-cleared / no-results* — machines online, nothing running; this is the Wind-down reading and is treated as a deliberate end state, visually distinct from first-use. Both are permitted only while the connection band reads connected.
- Error: `connecting` holds the last good rows and marks them stale; `disconnected` and `error` do the same and add the countdown and retry-now, with the needs-you count suppressed. A single unreachable machine marks only its own rows, leaving the rest of the board live.
...
- Empty: *user-cleared* — the request resolved while the operator was navigating in. Says what the answer was and where it came from (this browser, another tab, Telegram, or the harness), instead of rendering an answer row for a settled decision. A session with no turns yet shows the starting state, not a blank column.
- Error: the four connection phases, with the answer row **disabled** rather than merely warned while not live — on disk the failure is only thrown at send time (`client.svelte.ts` L1081: `'Not connected to the hub. Check that it is running, then try again.'`), which tells the operator after they committed. Machine-unresponsive is surfaced per session (L1676: `` `${label} got no answer in time. The machine may be offline.` ``). Request-already-answered-elsewhere resolves in place, naming the source.
...
- Empty: *first-use* — no machine has joined, so the surface states the prerequisite (join a machine) rather than offering a spawn control that would fail. With machines but no sessions yet, the empty state is the spawn control itself.
- Error: `No such project.` for an unresolvable id (L214), with a path back to the rail. Machine-offline is named against the machine with the others still selectable. A missing or unreadable directory is reported against the directory field before anything is created. Document read/write failures are already modelled separately on disk (`docsError`, `docError`, `claudeError`) and stay attached to their own block rather than failing the surface. Hub `disconnected` / `error` disables spawning and says why — spawns are never queued optimistically.
...
- Empty: *first-use* — the strongest empty state on disk and the model for the other six: `Nothing is watching yet` (L124) with three one-click templates (L140–152), which the file's own comment calls the onboarding (L27: *"The empty state is the onboarding: three ready-made rules, one click each."*). Three, not a catalogue: this is the branch where a longer list would cost the most **(Hick–Hyman 1952/1953)**. *User-cleared* — every rule deleted — offers the templates again rather than reverting to the first-use copy, which would misdescribe what happened.
...
- Empty: *first-use* — a brand-new rule, which is an empty form rather than an empty screen. Each section states what it is for, so the form is self-describing without a tour; the five sections are the chunking that keeps a long form legible.
...
- Empty: *first-use* — no machine has reported usage, rendered on disk as `No limit reading yet.` (L199). This must read differently from "reported, and spend is zero": they are opposite answers to the question the operator arrived with, and conflating them is a false all-clear.
- Error: on disk, upstream strings are surfaced directly as user-facing text — `'not signed in'`, `'token expired'`, or the raw string (L190–196). Per Nielsen's heuristic #9, help users recognize, diagnose, and recover from errors (1994), each must carry its recovery action, not just its diagnosis. Missing price data has a correct designed state already (L348: `No published price for …`) and is kept, because saying what is unknown beats reporting a confidently wrong total. Hub `disconnected` / `error` marks the reading stale with the countdown and retains it rather than blanking — a blank budget screen reads as "nothing is running", which is the false all-clear this design exists to prevent.
