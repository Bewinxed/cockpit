...
<!-- The structural and temporal design spec for Whiffle ("Whiffle"). Pairs with DESIGN.md
...
**Actor:** The fleet operator — the single person who owns every machine in this self-hosted deployment and every agent session running on it. Whiffle is single-tenant per deployment; there is exactly one operator role, never a team of distinct personas.
...
**Scope:** Future-state (Whiffle's dashboard, redesigned from the existing implementation — the plan mandates discarding and re-deriving the current UI in full). Current-state baseline is the existing dashboard (7 route surfaces already on disk, reported buggy CSS/layout — see plan `## Context`), plus a raw-terminal/tmux fallback habit the operator reaches for when that dashboard doesn't answer fast enough; the baseline is not the absence of any dashboard. Touchpoints included: the browser dashboard (desktop and mobile web) and the Telegram bridge. Excludes the `whiffle` daemon's own internals and harness-level CLI usage, which are not surfaces this design phase touches.
...
**Primary CTA:** *Allow* → session resumes. **(Fitts 1954)** — on a coarse pointer it is a full-width target in the thumb arc at the bottom of the viewport, not a small trailing button. The floor is WCAG 2.2 SC 2.5.8 Target Size (Minimum), Level AA — 24×24 CSS pixels (W3C Recommendation, 12 December 2024) — and this pair targets 44×44, which the codebase's own phone bar already treats as the floor (`lib/whiffle/ThumbBar.svelte`: *"44pt targets, the platform's floor for a thumb"*).
...
- **The spawn control already contains a project picker.** `lib/whiffle/SpawnPanel.svelte` L583–588 renders `<label for="spawn-project">Project (optional)</label>` over an input with `placeholder="Search projects…"`, filtering `whiffle.projects` by name and directory, with the empty case at L622: `No project by that name — pick a directory below instead.` Choosing one sets the project, machine and directory together. So the *task* in `## Journey` §Steer-or-spawn is already completable.
- `lib/whiffle/Sidebar.svelte` L1168 renders `<NewProjectPopover />` in the folders group label. That component states its own job: *"Names a directory so the rail has a folder for it before anything has run there. Every other folder in the rail is grown from live work, which leaves no way at all to add the checkout you have not started yet — this is it."*
- `lib/whiffle/JumpPalette.svelte` L27–38 builds a `'Projects'` group with `href: /project/${project.id}`.
- `lib/whiffle/SessionTabs.svelte` L490 and `lib/whiffle/FolderMenu.svelte` L86 both `goto(/project/${project.id})`.
...
**Compact is a fine-pointer affordance only.** Under `@media (pointer: coarse)` or at narrow widths, the compact scale relaxes: every row and control clears WCAG 2.2 SC 2.5.8 Target Size (Minimum), Level AA — 24×24 CSS pixels (W3C Recommendation, 12 December 2024) — and the approve/deny pair targets 44×44, matching the floor the codebase's own phone bar already uses (`lib/whiffle/ThumbBar.svelte`: *"44pt targets, the platform's floor for a thumb"*). Text inputs never render below 16px, or iOS zooms the viewport on focus.
...
7. **Jump palette (⌘K)** — the recognition-over-recall twin of the rail and the roster; groups on disk are `['Projects', 'Machines', 'Running sessions', 'Recent sessions']` (`lib/whiffle/JumpPalette.svelte` L27).
...
**Purpose:** Author or change one standing rule — what it watches for, what whiffle sends back, where it applies, and how it persists.
...
4. **What whiffle sends back** (L286).
...
- Empty: *first-use* — a brand-new rule, which is an empty form rather than an empty screen. Each section states what it is for (**Name it**, **What to watch for**, **What whiffle sends back**, **Where it applies**, **Making it stick**), so the form is self-describing without a tour; the five sections are the chunking that keeps a long form legible. Fields carry persistent labels, never placeholder-only (Rules: **Name**, **What to watch for**, **What whiffle sends back**, **Where it applies**, **Making it stick**). Validation failures are plain-language ("**Name this rule** fields can't be left blank — give it a name first."), marked at the failing field.
...
**Job story:** When an agent session is stalled or a run's spend surprises me and I suspect the cause spans more than one session, machine, or project — and no single agent in Whiffle can see across the fleet, because every one is bound to one repo, one machine, one task — I want to ask one surface a question no individual session's transcript can answer, so I can find the fleet-level cause and act on it without opening every session to assemble it by hand.

**Scope test (the assistant's only defensible territory):** every agent in Whiffle is bound to ONE session and cannot see outside it. The assistant's legitimate job is the class of question no single session can answer — fleet-spanning reasoning and lookups. Any question a session's own transcript answers is, by definition, answerable more directly and more truthfully by opening that session, and the assistant must not answer it.
...
| 2026-09-01 | **DW-9b.2's "zero silent actions" approval contract is overridden for the supervisor (rule kind "LLM") and per-session autopilot** — both act autonomously inside a session, with no per-action approval card. | This is not a reversal of the Phase 9a/9b kill test. DW-9b.2 governed the assistant **panel's** cross-session actions — spawn, clear a block, write a rule — reaching into sessions the operator was not already looking at, where consent had to be earned per action. The supervisor and autopilot are a different animal: opt-in only (a rule the operator wrote, or a per-session toggle the operator flipped) and confined to acting *inside* a session the operator already scoped in by arming it — consent moved from per-action to per-scope, it did not disappear. The boundaries that survive: permissions still escalate to the human only, the supervisor never answers a permission prompt; the panel's own cross-session actions keep the DW-9b.2 approval contract exactly as recorded above, untouched by this override; and every intervention is logged and labeled in the session transcript, origin `supervisor:*`, so nothing it does is silent even though nothing it does inside its scope is approved per-action. Implementing plan: `.unlazy-supervisor/PLAN.md` (Operator decisions §6). |
