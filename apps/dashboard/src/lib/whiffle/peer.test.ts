...
test("matching is unanchored and case-insensitive", () => {
  expect(matchPeers(peers, "BOARD").map((peer) => peer.label)).toEqual([
    "keeboard",
  ]);
  expect(matchPeers(peers, "center").map((peer) => peer.label)).toEqual([
    "center.ai",
  ]);
});

test("a machine name finds its sessions", () => {
  expect(matchPeers(peers, "obelisk").map((peer) => peer.label)).toEqual([
    "center.ai",
  ]);
});
