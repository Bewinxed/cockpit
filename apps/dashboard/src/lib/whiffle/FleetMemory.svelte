  ...
  <MemoryCard path="~/.claude/memories/{doc.path}" content={doc.content} save={saveDoc(doc.path)}>
    ...
    {#snippet actions()}
      <Tooltip.Root>
        ...
        <Tooltip.Content>Takes it off every machine that still has Whiffle's copy</Tooltip.Content>
      </Tooltip.Root>
    {/snippet}
    {#snippet footer()}
      {#if drifted.length > 0}
...
{:else}
  ...
  {#if hookFailed.length > 0}
    <div class="flex flex-col gap-[var(--space-2)]">
      <p class="max-w-prose text-caption text-muted-foreground">
        The documents above are put in front of a session by a SessionStart hook Whiffle registers.
  ...
  {:else}
    ...
    {#if drifted.length > 0}
      ...
      <p class="max-w-prose text-micro text-muted-foreground">
        A machine only ever gives back the copy Whiffle wrote it. One edited on the machine itself is left where it is, and says so here until you take it or replace it.
      </p>
    {/if}
  {/if}
{/if}
