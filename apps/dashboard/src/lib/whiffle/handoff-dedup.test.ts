...
function foldLiveInto(
  stored: import("./types").Message[],
  live: import("./types").Message[]
) {
  const merged = [...stored];
  for (const message of live) {
    if (mergePeerMessage(merged, message)) {
      continue;
    }
    merged.push(message);
  }
...
test("case 4 — an ordinary stored user turn stays a plain user turn", () => {
  const stored = mapTranscript("i1", [
    storedEntry("a normal message", "u-4"),
  ]).messages;
