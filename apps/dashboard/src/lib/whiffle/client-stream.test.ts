...
function harness(
  ...
  const host: StreamHost = {
    ...
    sendToHub: (message) => {
      ...
      if (options.throws) {
        throw new DOMException(options.throws, "InvalidStateError");
      }
      if (options.offline) {
        return false;
      }
    ...
    noteFailure: (record) =>
      failures.push(`${record.commandId}:${record.reason ?? ""}`),
    ...(options.timers
      ? {
  ...
  return {
    ...
    subscribes: () =>
      sent.filter(
        (message): message is StreamSubscribe =>
          message.type === "stream.subscribe"
      ),
  };
}
...
test("a late join adopts the hub sequence instead of demanding a replay from one", () => {
  ...
  h.receive(delta(4211));

  expect(h.applied.map((entry) => entry.seq)).toEqual([4211]);
  expect(h.subscribes()).toHaveLength(0);
});
...
test("a gap applies nothing and resubscribes with afterSeq at what is actually held", () => {
  ...
  expect(h.subscribes()).toEqual([
    { type: "stream.subscribe", sessionId: SESSION, afterSeq: 2 },
  ]);
});
...
test("sessions keep their own cursors: one session gapping does not disturb another", () => {
  ...
  expect(h.subscribes()).toEqual([
    { type: "stream.subscribe", sessionId: SESSION, afterSeq: 1 },
  ]);
});
...
test("capability is detected off any message that carries it, whichever one that is", () => {
  ...
  expect(noteCapabilities(h.state, { verb: "frames", payload: {} })).toBe(
    false
  );
  expect(
    noteCapabilities(h.state, { verb: "instances", capabilities: ["other.v9"] })
  ).toBe(false);
  expect(
    noteCapabilities(h.state, { verb: "instances", capabilities: [STREAM_V1] })
  ).toBe(true);
...
test("capability arriving after frames already flowed legacy switches cleanly, without a double ask", () => {
  ...
  expect(h.subscribes()).toEqual([
    { type: "stream.subscribe", sessionId: other },
  ]);
...
test("an event the hub could not fill still moves the cursor, so one blank is not a hole", () => {
  ...
  h.receive({
    type: "stream.event",
    event: { seq: 2, sessionId: SESSION, frame: null },
  });
  h.receive(delta(3));
...
test("a subscribe that could not leave is retried, not assumed to have been sent", () => {
  ...
  syncStreamSubscriptions(h.state, h.host, [SESSION]);
  expect(h.subscribes()).toEqual([
    { type: "stream.subscribe", sessionId: SESSION },
  ]);
 ...
 */
const settleStageOf = (kind: CommandKind): SettleStage =>
  kind === "send" ? "accepted" : "applied";
...
test("a dropped socket calls unanswered commands off and leaves settled ones alone", () => {
  ...
  expect(h.state.commands[outstanding]).toMatchObject({
    stage: "failed",
...
test("a result.error inside the shadow of this client's own interrupt command is recognisable — outside it, or after a refusal, it is not", () => {
  ...
  expect(interruptedRecently(h.state, "someone-else", submittedAt + 5000)).toBe(
    false
  );
  expect(interruptedRecently(h.state, SESSION, submittedAt + 60_000)).toBe(
    false
  );
  h.receive(ack("int-1", "failed", "no such session"));
  expect(interruptedRecently(h.state, SESSION, submittedAt + 5000)).toBe(false);
});
...
test("a command that died before dispatch is a failed record like any other — swept, readable, announced", () => {
  ...
  expect(
    sessionCommands(h.state, SESSION).map((record) => record.commandId)
  ).toEqual([commandId]);
  expect(h.failures).toEqual([
    `${commandId}:crypto.randomUUID is not a function`,
  ]);
...
test("a permission answer that fails is announced exactly once on each dialect, given a registry written before the submit", () => {
   ...
   * and never zero.
   */
  function run(dialect: "stream" | "legacy"): {
    toasts: number;
    cards: number;
  } {
    let parked = true;
    ...
    const cards =
      h.state.commands[commandId].stage === "failed" && parked ? 1 : 0;
    return { toasts, cards };
  }
