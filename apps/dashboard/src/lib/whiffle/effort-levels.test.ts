...
test("a model reaching every level reaches every stop", () => {
  ...
  expect(stops.map((stop) => stop.reachable)).toEqual([
    true,
    true,
    true,
    true,
...
test("nothing is hardcoded: a model naming only max reaches only max", () => {
  const stops = effortStops(
    model({ supportsEffort: true, supportedEffortLevels: ["max"] })
  );
  expect(
    stops.filter((stop) => stop.reachable).map((stop) => stop.value)
  ).toEqual(["max"]);
...
test("a model that claims effort but names no levels gets no control either", () => {
  ...
  expect(
    hasEffortScale(model({ supportsEffort: true, supportedEffortLevels: [] }))
  ).toBe(false);
});
...
test("high is the level the API answers on when nothing asks", () => {
  expect(
    EFFORT_LEVELS.filter((option) => option.apiDefault).map(
      (option) => option.value
    )
  ).toEqual(["high"]);
});
