...
test("groups start where their first command sits in the flat list", () => {
  ...
  expect(
    groupCommands(ordered).map(({ source, start, commands }) => ({
      source,
      start,
...
test("a list with no namespaces is one headingless group", () => {
  const groups = groupCommands(
    orderCommands([builtin("review"), builtin("agents")])
  );
...
test("a command opens a word at the caret, anywhere in the line", () => {
  ...
  expect(commandAt("have a look, /rev", 17)).toEqual({
    term: "rev",
    start: 13,
  });
});
...
const paletteOwnsEnter = (text: string, caret: number) => {
  ...
  return (
    token !== null &&
    filterCommands([builtin("review"), builtin("init")], token.term).length > 0
  );
};
...
test("a chosen command replaces its token and nothing around it", () => {
  ...
  expect(
    insertCommand("have a look, /rev", { start: 13 }, 17, "review")
  ).toEqual({
