<script lang="ts">
  ...
  let { done, total, size = 'md', style }: Props = $props();
...
<!-- Nothing planned draws nothing: an empty ring is a control that says a
     session has a plan it has not started, which is a different claim. -->
{#if total > 0}
  <svg
