...
const HOME_KEY = 'outpost-machine-home';
...
function guessHome(machineId: string): string | null {
  const cwds = [
    ...cockpit.instances.filter((row) => row.machineId === machineId).map((row) => row.cwd),
    ...cockpit.catalogOf(machineId).map((info) => info.cwd ?? ''),
  ];
...
async function probeHome(machineId: string): Promise<string | null> {
  const os = cockpit.machines.find((machine) => machine.machineId === machineId)?.os ?? '';
