...
>
  <Dialog.Content class="sm:max-w-lg">
    ...
    <form class="flex flex-col gap-3" onsubmit={finish}>
      {#if url}
        <a
          href={url}
          target="_blank"
          rel="noopener noreferrer"
          class="flex items-center justify-center gap-2 rounded-md bg-primary px-3 py-2 text-sm
      ...
      <input
        ...
        disabled={busy || !url}
        class="w-full rounded-md border border-border bg-background px-3 py-2 font-mono text-base
               sm:text-sm focus:border-ring focus:ring-2 focus:ring-ring/30 focus:outline-none
               disabled:opacity-60"
      />
