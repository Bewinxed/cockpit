 ...
 * - `apps/dashboard/src/lib/cockpit/stream.ts` — every client-side decision:
...
import type { Envelope, FramePayload } from '@cockpit/core';
...
import type { HubSocket, RegistryShape } from '../../../../../packages/hub/src/registry';
...
const DB_FILE = `/tmp/cockpit-e2e-${crypto.randomUUID()}.db`;
  ...
  return {
    ...
    dropDashboard: (socket) => {
      ...
      for (const [requestId, held] of requesters)
        if (held.id === socket.id) requesters.delete(requestId);
    },
    broadcast: (envelope) => {
...
if (!port) throw new Error('the hub did not listen');
...
const until = async (holds: () => boolean, label: string, budgetMs = 10_000): Promise<void> => {
...
const frameFor = (sessionId: string, ordinal: number): Record<string, unknown> => ({
  kind: 'frame',
  instanceId: sessionId,
  harness: 'claude',
  message: {
    type: 'assistant',
    message: { role: 'assistant', content: [{ type: 'text', text: `e-${ordinal}` }] },
  },
});
...
const ordinalOf = (frame: unknown): number => {
  const text = (
    frame as { message?: { message?: { content?: { text?: string }[] } } } | undefined
  )?.message?.message?.content?.[0]?.text;
  if (typeof text !== 'string' || !text.startsWith('e-')) return -1;
...
const openDaemon = async (machineId: string): Promise<Daemon> => {
  ...
  await ready;
  const send = (envelope: unknown): void => socket.send(JSON.stringify(envelope));
  send({
    verb: 'register',
    machineId,
    payload: { hostname: 'e2e', os: 'linux', auth: 'authenticated', instances: [] },
  });
  await until(() => registry.agent(machineId) !== undefined, `${machineId} to register`);
  return {
    received,
    ...
    answerControl: (requestId, ok, error) =>
      send({
        ...
        machineId,
        requestId,
        payload: { kind: 'control_result', requestId, ok, ...(error ? { error } : {}) },
      }),
    raw: send,
    close: () => socket.close(),
  };
...
const makeClient = (): Client => {
  ...
  const ingest = (
    ...
    if (source === 'legacy' && sessionId && streamCarries(state, sessionId, frame.kind)) return;
  ...
  const client: Client = {
    ...
    watch: (sessionIds) => {
      ...
      socket.send(
        JSON.stringify({ verb: 'subscribe', machineId: '', payload: { instanceIds: sessionIds } })
      );
    ...
    connect: async () => {
      ...
      if (client.watched.length > 0) client.watch(client.watched);
    },
    disconnect: async () => {
    ...
    transcript: (sessionId) =>
      applied
        .filter((entry) => entry.sessionId === sessionId && entry.frame.kind === 'frame')
...
const emitRun = async (
  ...
  for (let index = 0; index < count; index++) {
    ...
    if (index % 64 === 63) await Bun.sleep(1);
  }
  if (witness) {
    const target = count;
...
const session = (label: string): string => `${label}-${crypto.randomUUID().slice(0, 8)}`;
...
test('the hub advertises stream.v1 on the opening frame and the store flips capable', async () => {
  ...
  const first = client.inbox[0] as { verb?: string; payload?: { capabilities?: string[] } };
...
test('the handshake fires on the real opening frame: capabilities read from the payload nesting', async () => {
  ...
  expect((first.payload as { capabilities?: string[] }).capabilities).toEqual([STREAM_V1]);
...
test('the store consumes the raw top-level stream shapes the hub actually sends', async () => {
  ...
  client.watch([id]);
  await until(() => client.state.cursors[id]?.subscribed === true, 'the stream subscription');

  daemon.emit(id, 1);
  await until(() => client.transcript(id).length === 1, 'the first sequenced frame');
  ...
  expect(client.applied.at(-1)).toEqual({ sessionId: id, frame: frameFor(id, 1) as FramePayload, source: 'stream' });
...
test('a stream-followed session is not also delivered in the legacy dialect', async () => {
  ...
  await until(() => client.state.capable, 'capability');
  client.watch([id]);
  await until(() => client.state.cursors[id]?.subscribed === true, 'subscription');

  await emitRun(daemon, id, 1, 20, () => client.transcript(id).length);
  await quiet();

  expect(client.transcript(id)).toEqual(Array.from({ length: 20 }, (_, i) => i + 1));
  expect(client.applied.filter((e) => e.sessionId === id && e.source === 'legacy')).toEqual([]);
  await client.disconnect();
  daemon.close();
});
...
test(
  'G1 500 events with a mid-stream disconnect and resubscribe equal the uninterrupted control run',
  async () => {
    const daemon = await openDaemon(MACHINE);
    const id = session('g1');
    ...
    control.watch([id]);
    await until(() => control.state.cursors[id]?.subscribed === true, 'control subscription');

    const flaky = makeClient();
    await flaky.connect();
    await until(() => flaky.state.capable, 'flaky capability');
    flaky.watch([id]);
    await until(() => flaky.state.cursors[id]?.subscribed === true, 'flaky subscription');
    ...
    await emitRun(daemon, id, 181, 140, () => control.transcript(id).length - 180);
    expect(flaky.transcript(id).length).toBe(180);

    // Reconnect: the opening snapshot re-advertises, the flip resumes from 180.
    await flaky.connect();
    await until(() => flaky.transcript(id).length === 320, 'the replayed backlog');
...
test(
  'G2 a late join resuming from the start converges on the from-start follower',
  async () => {
    const daemon = await openDaemon(MACHINE);
    const id = session('g2');
    ...
    early.watch([id]);
    await until(() => early.state.cursors[id]?.subscribed === true, 'early subscription');
    ...
    await until(() => late.transcript(id).length === 300, 'the late join backlog');
...
test(
  'G2 a fresh late join with no afterSeq follows from now and converges on the tail',
  async () => {
    const daemon = await openDaemon(MACHINE);
    ...
    late.watch([id]);
    await until(() => late.state.cursors[id]?.subscribed === true, 'late subscription');
    await quiet();
...
const record = (client: Client, commandId: string): CommandRecord => {
  ...
  if (!found) throw new Error(`no command record for ${commandId}`);
  return found;
};
...
test('G3 a send command is accepted by the hub and its reflecting frame arrives on the stream', async () => {
  ...
  const relayed = daemon.received.find((e) => e.verb === 'send' && e.instanceId === id);
...
test('G3 a control command reaches applied only when the daemon answers it', async () => {
  ...
  await until(() => record(client, commandId).stage === 'accepted', 'the accepted ack');
  const control = daemon.received.find((e) => e.verb === 'control' && e.instanceId === id);
  ...
  await until(() => record(client, commandId).stage === 'applied', 'the applied ack');

  // The reply answered a command; it is not fleet news broadcast at everyone.
  expect(client.applied.some((e) => e.frame.kind === 'control_result')).toBe(false);
...
test('G3 a command the machine refuses ends failed with the reason, and stays failed', async () => {
  ...
  const control = daemon.received.find(
    (e) => e.verb === 'control' && (e.payload as { method?: string }).method === 'setModel'
  );
  daemon.answerControl(control?.requestId as string, false, 'that model is not installed');
  await until(() => record(client, commandId).stage === 'failed', 'the failed ack');
...
test('G3 a command for a machine that is not connected fails with the hub reason', async () => {
  ...
  await until(() => record(client, commandId).stage === 'failed', 'the failure ack');
...
test(
  'the backlog/live boundary holds when frames keep arriving during the resubscribe',
  async () => {
    const daemon = await openDaemon(MACHINE);
    ...
    for (let ordinal = 81; ordinal <= 140; ordinal++) daemon.emit(id, ordinal);
    await reconnected;
    await emitRun(daemon, id, 141, 40);
    await until(() => client.transcript(id).length === 180, 'every event either side of the seam');
    await quiet();
...
test(
  'a resume whose afterSeq is exactly the ring floor replays whole',
  async () => {
    const daemon = await openDaemon(MACHINE);
    ...
    witness.watch([id]);
    await until(() => witness.state.cursors[id]?.subscribed === true, 'witness subscription');
...
test(
  'a resume one event below the ring floor is refused with a reset and healed by a re-read',
  async () => {
    const daemon = await openDaemon(MACHINE);
    ...
    await client.connect();
    await until(() => client.rereads.length === 1, 'the honest refusal and its re-read');
    ...
    await quiet();
    expect(client.transcript(id)).toEqual([1, 2, 3, 4, 5, ...Array.from({ length: 10 }, (_, i) => 519 + i)]);
...
test('a gap invented on the wire is healed by exactly one resubscribe, not fifty', async () => {
  ...
  expect(
    client.outbox.filter((m) => m.type === 'stream.subscribe' && m.afterSeq === 10)
...
test('a broadcast kind the stream never carries still reaches the store on the legacy road', async () => {
  ...
  daemon.raw({
    ...
    payload: { kind: 'permission_request', instanceId: id, tool: 'Bash', input: {} },
  });
  await until(
    () => client.applied.some((e) => e.frame.kind === 'permission_request'),
    'the broadcast ask'
  );
  expect(client.applied.find((e) => e.frame.kind === 'permission_request')?.source).toBe('legacy');
  expect(client.state.cursors[id].streamed.has('permission_request')).toBe(false);
...
test('two sessions on one socket keep independent sequences', async () => {
  ...
  await until(
    () => client.transcript(first).length === 40 && client.transcript(second).length === 40,
    'both sessions'
  );
  await quiet();
...
test('two dashboards following one session see byte-identical streams', async () => {
  ...
  await emitRun(daemon, id, 1, 60, () => Math.min(a.transcript(id).length, b.transcript(id).length));
  await quiet();
