...
import type { FleetSyncReport } from '@cockpit/core';
import {
  CAUSE,
  causeOf,
  groupFaults,
...
describe('causeOf', () => {
  ...
  it('names an ssh fallback for a public repo', () => {
    ...
    expect(causeOf('git@github.com: Permission denied (publickey).')).toBe('ssh-refused');
    expect(causeOf('Could not read from remote repository.')).toBe('ssh-refused');
  ...
  it('reads the daemon’s own fixed sentences', () => {
    ...
    expect(causeOf('marketplace rtd is not linked')).toBe('marketplace-unlinked');
    expect(causeOf('edited on this machine — adopt it or overwrite')).toBe('drifted');
    expect(causeOf('the hub sent no files for this hash')).toBe('missing-bytes');
    expect(causeOf('unsafe path ../../etc/passwd')).toBe('unsafe-path');
    expect(causeOf('bound to a project this machine has no checkout for')).toBe('no-checkout');
    expect(causeOf('timed out after 120s')).toBe('timed-out');
    expect(causeOf('could not parse ~/.claude.json: Unexpected token')).toBe('unparsable');
    expect(causeOf('could not write ~/.claude.json: EACCES')).toBe('unwritable');
  ...
  it('reads an unrecognised HUB failure as a fetch that failed, not as unknown', () => {
    ...
    // is what names them.
    expect(causeOf('no marketplace called rtd in this fleet', 'hub')).toBe('unfetchable');
    expect(causeOf('github.com/x/y has no main to download', 'hub')).toBe('unfetchable');
...
describe('machineFaults', () => {
  it('finds every section of the report, including the three that used to be dropped', () => {
    const faults = machineFaults(
      'mac',
      report({
        plugins: { 'a@rtd': { state: 'failed', detail: "error: unknown option '--scope'" } },
        memoryDocs: { 'models/x.md': { state: 'failed', detail: 'edited on this machine — adopt it or overwrite' } },
        memoryHook: { state: 'failed', detail: 'could not parse ~/.claude/settings.json' },
        hooks: { h1: { state: 'failed', detail: 'bound to a project this machine has no checkout for' } },
      })
    );
    ...
    expect(faults.every((one) => one.machineId === 'mac' && one.origin === 'machine')).toBe(true);
  });
  ...
  it('ignores applied and removed rows', () => {
    const faults = machineFaults(
      'mac',
      report({
        mcp: { a: { state: 'applied' }, b: { state: 'removed', detail: 'kept: edited on this machine' } },
      })
    );
...
describe('hubFaults', () => {
  it('separates a hub resolve failure from any machine', () => {
    const faults = hubFaults(
      [{ name: 's', source: 'skills:a/b', enabled: true, error: 'a/b answered 404' }],
      [{ id: 'p@rtd', enabled: true, error: 'no marketplace called rtd in this fleet' }]
    );
    expect(faults).toHaveLength(2);
    expect(faults.every((one) => one.origin === 'hub' && one.machineId === undefined)).toBe(true);
    expect(faults.map((one) => one.cause)).toEqual(['unfetchable', 'unfetchable']);
  });

  it('leaves resolved rows alone', () => {
    expect(hubFaults([], [{ id: 'p@rtd', enabled: true, hash: 'abc', bytes: 12 }])).toEqual([]);
  });
});
...
describe('groupFaults', () => {
  it('collapses one cause on one machine into one problem', () => {
    const faults = machineFaults(
      ...
      report({
        plugins: {
          'a@rtd': { state: 'failed', detail: "error: unknown option '--scope'" },
          'b@rtd': { state: 'failed', detail: "error: unknown option '--scope'" },
          'c@rtd': { state: 'failed', detail: "error: unknown option '--scope'" },
        },
      })
    );
  ...
  it('keeps the same cause on two machines apart — a fault is attributable or it is nothing', () => {
    ...
    const groups = groupFaults([
      ...machineFaults('mac', report({ plugins: { 'a@rtd': { state: 'failed', detail } } })),
      ...machineFaults('obelisk', report({ plugins: { 'a@rtd': { state: 'failed', detail } } })),
    ]);
  ...
  it('puts hub faults first — nothing downstream is worth chasing until they are gone', () => {
    const groups = groupFaults([
      ...machineFaults('mac', report({ plugins: { 'a@rtd': { state: 'failed', detail: 'exited 1' } } })),
...
describe('readToolchain', () => {
  ...
  it('names the binary that ran and the newer one hiding behind it', () => {
    ...
    expect(reading?.used).toEqual({ path: '/usr/local/bin/claude', version: '2.0.32' });
  ...
  it('does not cry shadowing when the one in use is the newest', () => {
    const reading = readToolchain(
      report({
        toolchain: {
          claude: [
            { path: '/opt/homebrew/bin/claude', version: '2.1.251', used: true },
  ...
  it('will not compare versions it could not read', () => {
    const reading = readToolchain(
      report({ toolchain: { claude: [{ path: '/a/claude', used: true }, { path: '/b/claude' }] } })
    );
    expect(reading?.shadowed).toBe(false);
  });
});
