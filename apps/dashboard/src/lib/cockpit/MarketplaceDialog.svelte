<script lang="ts">
  ...
  async function link(event: SubmitEvent) {
    ...
    try { onsaved(await saveMarketplace(name.trim(), source.trim())); open = false; }
  ...
  <Dialog.Content class="rounded-2xl shadow-xl sm:max-w-lg">
    ...
    <form class="flex flex-col gap-3" onsubmit={link}>
      <label class="flex flex-col gap-1.5 text-caption">Source
        <Input bind:value={source} autocomplete="off" spellcheck="false" placeholder="anthropics/skills" class="font-mono text-sm md:text-sm" />
        <span class="text-micro">A GitHub <span class="font-mono">owner/repo</span>, a git URL, or a URL that ends in <span class="font-mono">marketplace.json</span>. Anthropic publishes <span class="font-mono">anthropics/skills</span> and <span class="font-mono">anthropics/claude-plugins-official</span>.</span>
      </label>
      <label class="flex flex-col gap-1.5 text-caption">Name
        <Input bind:value={name} autocomplete="off" spellcheck="false" aria-invalid={clash ? 'true' : undefined} placeholder="skills" class="font-mono text-sm md:text-sm" />
        <span class="text-micro">
          {#if clash}<span class="text-destructive">"{name.trim()}" is already linked.</span>
          {:else}What its plugins are installed as — <span class="font-mono">plugin@{name.trim() || 'name'}</span>.{/if}
        </span>
      </label>
