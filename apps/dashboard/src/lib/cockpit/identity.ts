 ...
 *
 * Usage: set `style={identityVar(cwd)}` on the element, then color through
 * the `identity-ink` / `identity-tint` / `identity-underline` utilities in
 * app.css, which read `--identity-h` and tune lightness per appearance.
 */

/** Ten identity hues (oklch). No hue below 40: a red folder reads as alarm. */
const HUES = [55, 85, 115, 145, 175, 205, 235, 265, 295, 330] as const;
...
/** Inline style assigning the element's identity hue custom property. */
export function identityVar(key: string): string {
  return `--identity-h: ${identityHue(key)}`;
}
