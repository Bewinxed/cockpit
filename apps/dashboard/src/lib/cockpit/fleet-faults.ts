 ...
 */
import type { FleetItemState, FleetPlugin, FleetSkillMeta, FleetSyncReport } from '@cockpit/core';
...
export const CAUSE: Record<FaultCause, CauseCopy> = {
  ...
  'cli-too-old': {
    ...
    why: 'The command cockpit ran uses an option this machine’s claude does not have, so the install never started.',
  ...
  'cli-missing': {
    ...
    why: 'Marketplaces and plugins are registered through the claude CLI, and this machine has none on any path cockpit can see.',
  ...
  drifted: {
    ...
    why: 'Cockpit only ever takes back the copy it wrote. Somebody changed this one on the machine itself, so it was left exactly where it is.',
  ...
  absent: {
    title: 'What cockpit wrote is no longer there',
  ...
  unparsable: {
    ...
    why: 'Cockpit will not overwrite a file it cannot parse, so nothing that file holds was converged.',
  ...
  unknown: {
    ...
    why: 'Cockpit has no reading of what came back, so the machine’s own words are below, untouched.',
