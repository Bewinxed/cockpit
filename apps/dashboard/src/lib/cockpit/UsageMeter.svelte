<script lang="ts">
   ...
   * folded into `cockpit.usageLimitsFor` — no polling. The opencode spend is
   * fetched on open, once, because it is a heavy REST aggregate rather than a
   * pushed number.
   */
  import { onMount } from 'svelte';
  import type { ClaudeLimits, LimitWindow } from '@cockpit/core';
  ...
  const limits: ClaudeLimits | null = $derived(
    machineId ? cockpit.usageLimitsFor(machineId) : cockpit.usageLimitsAny()
  );
