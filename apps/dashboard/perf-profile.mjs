...
await new Promise(r => {
  const handler = (ev) => {
    const msg = JSON.parse(ev.data);
    if (msg.method === 'Page.loadEventFired') { ws.removeEventListener('message', handler); r(); }
  };
...
console.log(`  Recalc style duration: ${(m1.RecalcStyleDuration * 1000).toFixed(1)}ms`);
...
if (heap1) console.log(`  JS heap: ${heap1.used}MB / ${heap1.total}MB`);
...
for (const t of domStats.tabs) {
  console.log(`    ${t.selected === 'true' ? '→' : ' '} "${t.label}" [${t.dataTab.slice(0,8)}]`);
}
...
// ── 6. TAB SWITCH ───────────────────────────────────────────────────
console.log('\n── 6. TAB SWITCH ──');
const otherTabs = domStats.tabs.filter(t => t.dataTab !== 'none' && t.selected !== 'true');
if (otherTabs.length > 0) {
  ...
  console.log(`  Post-switch FPS: ${switchFps.fps} (jank frames: ${switchFps.jank})`);
} else {
...
const canScroll = await evaluate(`!!document.querySelector('[tabindex="-1"].overflow-y-auto')`);
...
if (canScroll) {
  ...
  const collected = new Promise(r => {
    const handler = (ev) => {
      ...
      if (msg.method === 'Tracing.tracingComplete') { ws.removeEventListener('message', handler); r(); }
    };
    ws.addEventListener('message', handler);
  });
  ...
  if (longLayouts.length) console.log(`    Worst: ${(Math.max(...longLayouts.map(e=>e.dur))/1000).toFixed(1)}ms`);
  console.log(`  Style recalcs: ${recalcs.length}  (>5ms: ${longRecalcs.length})`);
  if (longRecalcs.length) console.log(`    Worst: ${(Math.max(...longRecalcs.map(e=>e.dur))/1000).toFixed(1)}ms`);
  console.log(`  CompositeLayers: ${composites.length}`);

  const m3 = await getMetrics();
  console.log(`  Post-scroll script duration: ${(m3.ScriptDuration * 1000).toFixed(1)}ms`);
} else {
...
if (finalHeap) console.log(`  JS heap: ${finalHeap.used}MB / ${finalHeap.total}MB`);
