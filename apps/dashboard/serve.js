 ...
 * that is not a style choice. Bun does not emit `'upgrade'` on an outgoing
 * `http.request`: it hands the 101 back as an ordinary `'response'`, so a proxy
 * written against the Node event — the dev plugin's — silently never fires and
 * the browser waits forever for a handshake nobody completed. After the
 * handshake a websocket proxy is only bytes in both directions anyway, so the
 * request line and headers are re-issued verbatim over a plain TCP connection
 * and the hub's own 101 passes straight back. Nothing here has to agree with a
 * runtime about what an upgrade is.
 */
