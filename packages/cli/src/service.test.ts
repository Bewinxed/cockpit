...
describe("sessiond joins the service set", () => {
  test("it is a service id, ordered before the agent that dials it", () => {
    ...
    expect(SERVICE_IDS.indexOf("sessiond")).toBeLessThan(
      SERVICE_IDS.indexOf("agent")
    );
  });
  ...
  test("no unit sets KillMode — the cgroup default is the decision", () => {
    for (const id of SERVICE_IDS) {
      expect(serviceDefinition(id, "prod", "systemd")).not.toContain(
        "KillMode"
      );
    }
  });
...
describe("the agent orders after sessiond", () => {
  ...
  test("a service with no sibling ordering gets no comment", () => {
    ...
    expect(serviceDefinition("hub", "prod", "launchd")).not.toContain(
      "<!-- ordering"
    );
    expect(serviceDefinition("sessiond", "prod", "launchd")).not.toContain(
      "<!-- ordering"
    );
  });
});
...
describe("restarting sessiond goes through the same gate as the agent", () => {
  test("busy refuses, and says what a sessiond restart costs", () => {
    ...
    if (decision.kind !== "refuse") {
      throw new Error(`expected a refusal, got ${decision.kind}`);
    }
    expect(decision.reason).toContain(
      "the sessiond on this machine is mid-turn in 3 sessions"
    );
    expect(decision.reason).toContain(
      "kills the harness children in its cgroup"
    );
  ...
  test("--when-idle waits instead of refusing", () => {
    expect(
      restartDecision({ busy: 2, whenIdle: true, force: false, id: "sessiond" })
    ).toEqual({
      kind: "wait",
      busy: 2,
    });
  ...
  test("--force goes, busy or unknown", () => {
    expect(
      restartDecision({ busy: 9, whenIdle: false, force: true, id: "sessiond" })
    ).toEqual({
      kind: "go",
    });
    expect(
  ...
  test("the agent wording is unchanged when no id is given", () => {
    ...
    expect(decision.reason).toContain(
      "the agent on this machine is mid-turn in 1 session"
    );
...
describe("the ad-hoc pid ledger", () => {
  ...
  test("the ledger round-trips and rejects malformed entries", async () => {
    ...
    try {
      ...
      expect(await readSessiondLedger(path)).toEqual([
        { pid: 7, startTicks: "123" },
      ]);

      await Bun.write(
        path,
        JSON.stringify([{ pid: "seven" }, 3, { pid: 8, startTicks: "4" }])
      );
      expect(await readSessiondLedger(path)).toEqual([
        { pid: 8, startTicks: "4" },
      ]);
  ...
  test("a sweep whose entries all fail the start-time match kills nothing", async () => {
    ...
    try {
      ...
      await writeSessiondLedger(
        [{ pid: 1, startTicks: "not-init-start-time" }],
        path
      );
      const notes: string[] = [];
      const killed = await sweepSessiondOrphans(
