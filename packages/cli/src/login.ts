...
export const login = async (): Promise<void> => {
  ...
  const flow = Bun.spawn(SETUP_TOKEN, { stdio: ['inherit', 'inherit', 'inherit'] });
  if ((await flow.exited) !== 0) {
    throw new LoginError('`claude setup-token` did not finish; nothing was saved.');
  }

  const pasted = prompt('\nPaste the token it printed:');
  if (!pasted?.trim()) throw new LoginError('nothing pasted; nothing was saved.');
...
export const saveToken = async (token: string): Promise<void> => {
  ...
  if (!trimmed.startsWith(TOKEN_PREFIX)) {
    throw new LoginError(`that does not look like a Claude Code token (expected ${TOKEN_PREFIX}…).`);
  }
...
export const clearToken = async (): Promise<boolean> => {
  ...
  if (!config?.claudeToken) return false;
