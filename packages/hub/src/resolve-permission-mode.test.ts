...
test("a nested delegate inherits the bypassing root", () => {
  expect(resolveDelegatePermissionMode(rootBypass, "delegate-1")).toBe(
    "bypassPermissions"
  );
  expect(resolveDelegatePermissionMode(rootBypass, "delegate-2")).toBe(
    "bypassPermissions"
  );
});
...
test("the immediate parent does not mask the root", () => {
  ...
  expect(resolveDelegatePermissionMode(rows, "delegate-2")).toBe(
    "bypassPermissions"
  );
});
