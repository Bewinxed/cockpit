...
let nextVerdict: ScriptedVerdict = { verdict: 'silent', message: '', note: 'default' };
let fakeDelay = 0;
let fakeStatus = 200;
...
const fakeOpenAI = Bun.serve({
  ...
  fetch: async (req) => {
    ...
    if (fakeDelay > 0) await Bun.sleep(fakeDelay);
...
const openDaemon = async (machineId: string): Promise<Daemon> => {
  ...
  send({
    verb: 'register',
    machineId,
    payload: { hostname: 'sv-e2e', os: 'linux', auth: 'authenticated', instances: [] },
  });
...
const completeTurn = (daemon: Daemon, instanceId: string, text: string): void => {
...
const openDashboard = async (): Promise<{
  ...
  socket.send(JSON.stringify({ verb: 'subscribe', machineId: '', payload: { instanceIds: [] } }));
  await Bun.sleep(20);
...
describe('supervisor e2e', () => {
  test('3a: reply verdict → agent socket receives send envelope with correct origin and body', async () => {
    ...
    nextVerdict = { verdict: 'reply', message: replyMessage, note: 'no evidence' };

    // Clear the daemon's received buffer.
    daemon.received.length = 0;

    completeTurn(daemon, instanceId, 'I have completed all the work successfully.');
    ...
    expect(req.system).toContain('Watch for false done claims and demand proof.');
  ...
  test('3b: autopilot takes precedence over LLM rule', async () => {
    ...
    const autopilotPrompt = 'Keep the session moving and escalate real decisions.';
    const res = await fetch(`http://localhost:${hubPort}/api/autopilot/${instanceId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  test('3c: escalate verdict → no send to agent, supervisor_event on dashboard, log row exists', async () => {
    ...
    nextVerdict = { verdict: 'escalate', message: 'Needs human review.', note: 'drift' };
    ...
    const eventFrame = dashboard.inbox.find((m) => {
      const payload = (m as { payload?: { kind?: string; instanceId?: string } }).payload;
      return payload?.kind === 'supervisor_event' && payload?.instanceId === instanceId;
  ...
  test('3d: turn ending mid-evaluation → skipped; frames keep flowing during delayed verdict', async () => {
    ...
    const probeRes = await fetch(`http://localhost:${hubPort}/api/supervisor/events?instanceId=${instanceId}&limit=1`);
    ...
    const instanceRes = await fetch(`http://localhost:${hubPort}/api/instances`);
    ...
    const events = (await eventsRes.json()) as SupervisorEvent[];
    const skipped = events.find((e) => e.verdict === 'skipped' && e.note === 'in-flight');
  ...
  test('3e: consecutive cap — fourth reply forced to escalate, mute holds until a human send', async () => {
    ...
    await until(
      () => {
        ...
        return events.some(
          (e) => e.verdict === 'escalate' && e.note?.includes('consecutive-reply limit'),
        );
      },
