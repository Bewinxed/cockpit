...
test("a delegate's spawn title is what the listing reads back", () => {
  ...
  expect(rowOf("delegate-1")?.title).toBe(
    "Carry the delegate brief headline end to end"
  );
});
...
test("a session nobody named is called after what it was first asked", () => {
  db.noteDerivedTitle(
    "plain-1",
    deriveTitleFromFirstMessage("  Fix the tab strip\n\nplease  ")
  );
...
test("a spawn title outranks the first message: the derived name is never written over it", () => {
  expect(
    db.noteDerivedTitle("delegate-1", "Whatever the delegate was first told")
  ).toBe(false);
