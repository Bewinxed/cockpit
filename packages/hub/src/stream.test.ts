...
import { createStreamHub, RING_SIZE, type ControlResultFrame, type StreamPorts } from './stream';
...
const fakeSocket = (id: string, throwOnSend = false): FakeSocket => {
  ...
  return {
    ...
    sent,
    send: (data: unknown) => {
      if (throwOnSend) throw new Error('socket is closed');
      sent.push(data);
    },
  };
};
...
type Backlog = { type: 'stream.backlog'; sessionId: string; events: SessionStreamEvent[] };

/** Every message this socket was sent, in order, with its `type` narrowed. */
const typed = <T extends { type: string }>(socket: FakeSocket, type: T['type']): T[] =>
...
test('the sequencer stamps 1..N per session and counts each session on its own', () => {
  ...
  expect(hub.sequence('a', { n: 4 })).toEqual({ seq: 4, sessionId: 'a', frame: { n: 4 } });
});

test('a session is sequenced whether or not anybody is following it', () => {
  const { hub } = harness();
  for (let n = 0; n < 3; n++) hub.sequence(SESSION, { n });

  const socket = fakeSocket('late');
  hub.handleClientMessage(socket, { type: 'stream.subscribe', sessionId: SESSION });
  ...
  expect(typed<Delta>(socket, 'stream.event')).toEqual([
    { type: 'stream.event', event: { seq: 4, sessionId: SESSION, frame: { n: 3 } } },
  ]);
});

test('a resume inside the ring answers with a contiguous backlog from afterSeq + 1', () => {
  const { hub } = harness();
  for (let n = 1; n <= 5; n++) hub.sequence(SESSION, { n });

  const socket = fakeSocket('resumer');
  hub.handleClientMessage(socket, { type: 'stream.subscribe', sessionId: SESSION, afterSeq: 2 });
...
test('a client already current gets an empty backlog rather than silence', () => {
  ...
  hub.handleClientMessage(socket, { type: 'stream.subscribe', sessionId: SESSION, afterSeq: 1 });

  expect(socket.sent).toEqual([{ type: 'stream.backlog', sessionId: SESSION, events: [] }]);
});

test('a resume older than the ring answers with a reset naming the next seq', () => {
  const { hub } = harness();
  for (let n = 1; n <= RING_SIZE + 10; n++) hub.sequence(SESSION, { n });
  ...
  hub.handleClientMessage(socket, { type: 'stream.subscribe', sessionId: SESSION, afterSeq: 9 });
...
test('the ring wraps without dropping a seq: the last RING_SIZE events still replay whole', () => {
  ...
  hub.handleClientMessage(socket, { type: 'stream.subscribe', sessionId: SESSION, afterSeq: 10 });

  const [backlog] = typed<Backlog>(socket, 'stream.backlog');
  expect(backlog?.events.length).toBe(RING_SIZE);
  expect(backlog?.events[0]).toEqual({ seq: 11, sessionId: SESSION, frame: { n: 11 } });
  ...
  expect(seqs.every((seq, index) => index === 0 || seq === seqs[index - 1]! + 1)).toBe(true);
});
...
test('a resume from a dead epoch — afterSeq past the head — answers with a reset, not a backlog', () => {
  ...
  for (let n = 1; n <= 3; n++) hub.sequence(SESSION, { n });

  const socket = fakeSocket('stale-epoch');
  hub.handleClientMessage(socket, { type: 'stream.subscribe', sessionId: SESSION, afterSeq: 900 });

  expect(socket.sent).toEqual([{ type: 'stream.reset', sessionId: SESSION, nextSeq: 4 }]);
});
...
test('a resume the client cannot express — a malformed afterSeq — answers with a reset', () => {
  ...
  expect(socket.sent).toEqual([{ type: 'stream.reset', sessionId: SESSION, nextSeq: 2 }]);
});
...
test('backlog and live delivery meet with no duplicate and no gap', () => {
  ...
  hub.handleClientMessage(socket, { type: 'stream.subscribe', sessionId: SESSION, afterSeq: 2 });
  for (let n = 6; n <= 8; n++) hub.sequence(SESSION, { n });

  // What the client actually applies, in the order it arrives: the backlog's
  // events flattened, then every live delta.
  const applied: number[] = [];
  for (const message of socket.sent as { type: string; events?: { seq: number }[]; event?: { seq: number } }[]) {
    if (message.type === 'stream.backlog') applied.push(...message.events!.map((e) => e.seq));
    if (message.type === 'stream.event') applied.push(message.event!.seq);
  }
...
test('two sockets follow one session and each is told every event exactly once', () => {
  ...
  hub.handleClientMessage(first, { type: 'stream.subscribe', sessionId: SESSION, afterSeq: 1 });
  hub.handleClientMessage(second, { type: 'stream.subscribe', sessionId: SESSION });
...
test('a streamed session is subtracted from the legacy subscription set, both orders', () => {
  ...
  expect(hub.noteLegacySubscriptions(socket, [SESSION, 'other'])).toEqual([SESSION, 'other']);
  hub.handleClientMessage(socket, { type: 'stream.subscribe', sessionId: SESSION });
  expect(legacy.get(socket.id)).toEqual(['other']);

  // And a later re-statement of the whole set stays subtracted.
  expect(hub.noteLegacySubscriptions(socket, [SESSION, 'other', 'third'])).toEqual([
    'other',
    'third',
  ]);
});
...
test('a session gone quiet loses its replay window but never its sequence', () => {
  ...
  hub.handleClientMessage(behind, { type: 'stream.subscribe', sessionId: SESSION, afterSeq: 3 });
  expect(behind.sent).toEqual([{ type: 'stream.reset', sessionId: SESSION, nextSeq: 7 }]);

  // And what survived the sweep still replays.
  const current = fakeSocket('current-after-sweep');
  hub.handleClientMessage(current, { type: 'stream.subscribe', sessionId: SESSION, afterSeq: 5 });
  expect(typed<Backlog>(current, 'stream.backlog')[0]?.events.map((e) => e.seq)).toEqual([6]);
});
...
test('a command whose relay refuses it fails rather than sitting on an accepted it never earned', () => {
  ...
  expect(h.hub.settleCommand('any', { kind: 'control_result', requestId: 'any', ok: true })).toBe(
    false
  );
});
...
test('a command from a socket that follows no stream is still acknowledged, twice over', () => {
  ...
  const requestId = (h.relayed[0]?.envelope.payload as ControlPayload).requestId;
  expect(h.hub.settleCommand(requestId, { kind: 'control_result', requestId, ok: true })).toBe(true);
...
test('the same permission answered twice in flight is refused the second time', () => {
  ...
  h.hub.settleCommand('ask-1', { kind: 'control_result', requestId: 'ask-1', ok: true });
...
test('a command whose socket dies before the daemon answers is consumed, not broadcast', () => {
  ...
  const result: ControlResultFrame = { kind: 'control_result', requestId, ok: true };
...
const makeTestRegistry = (): RegistryShape & { readonly forwarded: Envelope[] } => {
...
const openSocket = async (path: string): Promise<Wire> => {
  ...
  return {
    ...
    next: async (match, label) => {
      ...
      throw new Error(`timed out waiting for ${label}; inbox: ${JSON.stringify(inbox)}`);
    },
    close: () => socket.close(),
  };
};

const isAck = (commandId: string, stage: string) => (message: Record<string, unknown>) =>
  message.type === 'command.ack' && message.commandId === commandId && message.stage === stage;
...
test('the first message a dashboard receives carries the hub capabilities', async () => {
  ...
  const first = await dashboard.next((m) => m.verb === 'frames', 'the opening snapshot');

  const payload = first.payload as { kind: string; capabilities?: string[]; instances?: unknown[] };
...
test('a live frame reaches a stream subscriber sequenced, and a legacy subscriber unchanged', async () => {
  ...
  legacy.send({ verb: 'subscribe', machineId: '', payload: { instanceIds: [instanceId] } });
  modern.send({ verb: 'subscribe', machineId: '', payload: { instanceIds: [instanceId] } });
  ...
  agent.send({ verb: 'frames', machineId: MACHINE, instanceId, payload: frame });

  const delta = await modern.next((m) => m.type === 'stream.event', 'a sequenced delta');
  expect(delta.event).toEqual({ seq: 1, sessionId: instanceId, frame });

  const relayed = await legacy.next(
    (m) => m.verb === 'frames' && (m.payload as { kind?: string }).kind === 'frame',
    'the legacy relay'
  );
  ...
  expect(
    modern.inbox.some((m) => m.verb === 'frames' && (m.payload as { kind?: string }).kind === 'frame')
  ).toBe(false);
...
test('a send command reaches the machine as the relay op it names and stops at accepted', async () => {
  ...
  const relayed = registry.forwarded.slice(before).find((e) => e.verb === 'send');
  expect(relayed?.instanceId).toBe(SESSION);
  expect(relayed?.machineId).toBe(MACHINE);
  expect((relayed?.payload as SendPayload).message).toEqual(message as SendPayload['message']);
...
test('each control command reaches the machine as its own method, and only its own', async () => {
  ...
  const cases: { kind: string; payload: Record<string, unknown>; method: string; args: unknown[] }[] =
    [
      { kind: 'interrupt', payload: { args: [] }, method: 'interrupt', args: [] },
      { kind: 'set-model', payload: { args: ['opus'] }, method: 'setModel', args: ['opus'] },
      {
        kind: 'set-permission-mode',
        payload: { args: ['plan'] },
      ...
      { kind: 'set-effort', payload: { args: ['high'] }, method: 'setEffort', args: ['high'] },
      {
        kind: 'permission.answer',
  ...
  for (const [index, entry] of cases.entries()) {
    ...
    await dashboard.next(isAck(`ctl-${index}`, 'accepted'), `${entry.kind} accepted`);
  }
  ...
  expect(controls.map((payload) => payload.method)).toEqual(cases.map((entry) => entry.method));
  expect(controls.map((payload) => payload.args)).toEqual(cases.map((entry) => entry.args));
  expect(controls.every((payload) => payload.instanceId === SESSION)).toBe(true);
...
test('a control command is applied when the daemon confirms it, in its own words when it will not', async () => {
  ...
  agent.send({
    ...
    payload: { kind: 'control_result', instanceId: SESSION, requestId: good!.requestId, ok: true },
  });
  agent.send({
    verb: 'frames',
    machineId: MACHINE,
  ...
  const failed = await dashboard.next(isAck('refused-1', 'failed'), 'the failed ack');
  ...
  expect(
    dashboard.inbox.some(
      (m) => m.verb === 'frames' && (m.payload as { kind?: string }).kind === 'control_result'
    )
  ).toBe(false);
...
test('a command ack never steals the reply a legacy dashboard is waiting for', async () => {
  ...
  legacy.send({
    ...
    instanceId: SESSION,
    requestId,
    payload: { instanceId: SESSION, requestId, method: RESOLVE_PERMISSION, args },
  });
  await Bun.sleep(30);
  modern.send({
    type: 'command',
  ...
  agent.send({
    ...
    instanceId: SESSION,
    requestId,
    payload: { kind: 'control_result', instanceId: SESSION, requestId, ok: true },
  });
...
test('a reconnecting client resumes the stream across the socket it lost', async () => {
  ...
  const frameAt = (n: number) => ({
    ...
    instanceId,
    harness: 'claude',
    message: { type: 'assistant', message: { role: 'assistant', content: [] }, n },
  });
  agent.send({ verb: 'frames', machineId: MACHINE, instanceId, payload: frameAt(1) });
  ...
  for (let n = 2; n <= 4; n++)
    agent.send({ verb: 'frames', machineId: MACHINE, instanceId, payload: frameAt(n) });
  await Bun.sleep(30);
  ...
  const backlog = await second.next((m) => m.type === 'stream.backlog', 'the resume backlog');
  expect((backlog.events as { seq: number }[]).map((event) => event.seq)).toEqual([2, 3, 4]);

  agent.send({ verb: 'frames', machineId: MACHINE, instanceId, payload: frameAt(5) });
  const delta = await second.next((m) => m.type === 'stream.event', 'the delta after the backlog');
