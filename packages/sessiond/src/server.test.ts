...
import type { SessiondServerMessage } from "@whiffle/core/sessiond";
import { SessiondServer } from "./server";

/** Every socket in this file lives in a scratch dir; never the real endpoint. */
const scratch = (): string =>
  join(mkdtempSync(join(tmpdir(), "sessiond-test-")), "sessiond.sock");
...
class Client {
  ...
  constructor(readonly socket: Socket) {
    ...
    socket.on("data", (chunk: string) => {
      ...
      while (nl >= 0) {
        this.received.push(
          JSON.parse(this.#buffer.slice(0, nl)) as SessiondServerMessage
        );
...
describe("sessiond verbs", () => {
  test("G1: spawn/write/subscribe/list/stdin_end round-trip against a real child", async () => {
    ...
    expect(listed.procs).toMatchObject([
      { procId: "p1", alive: true, head: 2 },
    ]);
  ...
  test("G2: ring overflow answers a stale cursor with reset, never a spliced stream", async () => {
    ...
    await client.waitFor(
      (m) => m.type === "ack" && m.commandId === "o1",
      "spawn ack"
    );
    await client.waitFor(
      (m) => m.type === "proc.exit" && m.procId === "flood",
      "flood exit"
    );
    ...
    const backlog = await client.waitFor<{
  ...
  test("G3: a re-delivered commandId is re-acked, not re-executed", async () => {
    ...
    await Bun.sleep(150);
    const acks = client.received.filter(
      (m) => m.type === "ack" && m.commandId === "dup"
    );
  ...
  test("G4: children survive a socket drop and the backlog replays gap-free from afterSeq", async () => {
    ...
    await first.waitFor(
      (m) => m.type === "proc.line" && m.event.seq === 3,
      "three lines"
    );
    ...
    await second.waitFor(
      (m) => m.type === "ack" && m.commandId === "k3",
      "write ack"
    );
    await Bun.sleep(100);
    second.send({ type: "subscribe", procId: "keep", afterSeq: 3 });
    const backlog = await second.waitFor<{
      events: Array<{ seq: number; data: string }>;
    }>(
...
describe("sessiond socket and drain", () => {
  ...
  test("drain is stdin-EOF, then grace, then SIGKILL", async () => {
    ...
    await client.waitFor(
      (m) => m.type === "ack" && m.commandId === "d2",
      "spawn acks"
    );

    await server.drain(500);
    const polite = await client.waitFor<{
      exitCode: number | null;
      signal: string | null;
    }>((m) => m.type === "proc.exit" && m.procId === "polite", "polite exit");
...
describe("attaching before the spawn", () => {
  ...
  test("a pre-spawn subscriber gets an empty backlog and then every line", async () => {
    ...
    await client.waitFor(
      (m) => m.type === "proc.exit" && m.procId === "early",
      "exit"
    );

    expect(client.lines("early").map((line) => line.data)).toEqual([
      "line 1",
      "line 2",
      "line 3",
    ]);
