...
const check = (name: string, pass: boolean, diag?: unknown): void => {
  if (pass) console.log(`PASS ${name}`);
    ...
    console.log(`DIAG ${JSON.stringify({ assertion: name, ...(diag as object) })}`);
  }
};
...
const sendContentOf = (env: Envelope): string | undefined => {
  const content = (env.payload as { message?: { message?: { content?: unknown } } }).message
    ?.message?.content;
  return typeof content === 'string' ? content : undefined;
};

const controlPayloadOf = (env: Envelope): {
  instanceId?: string;
  requestId?: string;
  method?: string;
  args?: unknown[];
...
try {
  ...
  if (up !== true) throw new Error('the isolated hub never came up');

  const agentMessages: Envelope[] = [];
  const dashboardMessages: Envelope[] = [];

  const connect = (url: string, onMessage: (env: Envelope) => void): Promise<WebSocket> => {
  ...
  const dashboard = await connect(`${WS}/ws/dashboard`, (env) => dashboardMessages.push(env));

  const send = (ws: WebSocket, envelope: Envelope): void => ws.send(JSON.stringify(envelope));

  send(agent, {
    verb: 'register',
    machineId: M,
    payload: { hostname: 'proof-host', os: 'linux', auth: 'authenticated', instances: [] },
  });
  const registered = await waitFor(async () => {
    const agents = (await fetch(`${BASE}/api/agents`).then((r) => r.json())) as {
      machineId: string;
    }[];
    return agents.some((a) => a.machineId === M) ? true : undefined;
  }, 5_000);
  check('agent registers', registered === true);
  if (registered !== true) throw new Error('the fake agent never registered');

  const spawnInstance = (instanceId: string, cwd: string, parentId?: string): void =>
    send(dashboard, {
      verb: 'spawn',
      machineId: M,
      instanceId,
  ...
  const waitForInstance = (id: string): Promise<boolean> =>
    waitFor(async () => {
      const rows = (await fetch(`${BASE}/api/instances`).then((r) => r.json())) as {
        id: string;
  ...
  const askQuestion = (instanceId: string, requestId: string, question: string, options: string[]): void =>
    send(agent, {
      verb: 'frames',
      machineId: M,
      instanceId,
  ...
  const dA = await waitFor(async () => {
    ...
    return rows.find((r) => r.id === D_A)?.parentInstanceId === P_A ? true : undefined;
  }, 5_000);
  ...
  check(
    ...
    (aContent ?? '').includes(`[delegate-ask instance=${D_A} request=${REQ_A}]`),
    { content: aContent }
  );
  check('A: send renders the question text verbatim', (aContent ?? '').includes('Which option?'));
  check(
    'A: send renders the option labels verbatim',
    (aContent ?? '').includes('Option A') && (aContent ?? '').includes('Option B'),
    { content: aContent }
  );

  const aDash = await waitFor(
    () => dashboardMessages.find((m) => m.verb === 'frames' && m.requestId === REQ_A),
    5_000
  );
  check('A: dashboard copy carries routedTo=parent', aDash !== undefined && routedToOf(aDash) === 'parent', {
    routedTo: aDash ? routedToOf(aDash) : undefined,
  });

  const aPending = (await fetch(`${BASE}/api/pending`).then((r) => r.json())) as Envelope[];
  check('A: /api/pending still lists the ask', aPending.some((p) => p.requestId === REQ_A));
  ...
  const bDash = await waitFor(
    () => dashboardMessages.find((m) => m.verb === 'frames' && m.requestId === REQ_B),
    5_000
  );
  check('B: fallback ask broadcast untagged', bDash !== undefined && routedToOf(bDash) === undefined, {
    routedTo: bDash ? routedToOf(bDash) : undefined,
  });
  check(
    'B: no peer send synthesized for the fallback',
    !agentMessages.some((m) => m.verb === 'send' && (sendContentOf(m) ?? '').includes(`instance=${D_B}`))
  );
  ...
  const reply = await actions.answerDelegate(D_A, REQ_A, { 'Which option?': 'Option A' });
  check('C: answerDelegate resolves its own delegate', reply.includes(D_A) || reply.includes('delegate-a'));
  ...
  check(
    ...
    control?.instanceId === D_A && control?.requestId === REQ_A && cPayload?.instanceId === D_A,
    { control }
  );
  check('C: control method is resolvePermission', cPayload?.method === 'resolvePermission', {
    method: cPayload?.method,
  });
  const args = cPayload?.args as unknown[] | undefined;
  const result = args?.[1] as { updatedInput?: { answers?: Record<string, string> } } | undefined;
  ...
  check('C: control claims the parent as caller', cPayload?.from === P_A, { from: cPayload?.from });
  ...
  const dC = await waitFor(async () => {
    ...
    return rows.find((r) => r.id === D_C)?.parentInstanceId === P_C ? true : undefined;
  }, 5_000);
  ...
  const dRouted = await waitFor(
    () =>
      dashboardMessages.find(
        (m) => m.verb === 'frames' && m.requestId === REQ_C && routedToOf(m) === 'parent'
      ),
    5_000
  );
  check('D: ask routed while the parent is live', dRouted !== undefined);

  send(dashboard, { verb: 'stop', machineId: M, instanceId: P_C, payload: { instanceId: P_C } });
  const dEscalated = await waitFor(
    () =>
      dashboardMessages.filter((m) => m.verb === 'frames' && m.requestId === REQ_C).some(
        (m) => routedToOf(m) === undefined
      )
        ? true
        : undefined,
    5_000
