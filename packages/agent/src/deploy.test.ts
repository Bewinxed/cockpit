...
const sh = async (cwd: string, ...argv: string[]): Promise<string> => {
  ...
  if (ran.exitCode !== 0) {
    throw new Error(`${argv.join(' ')} in ${cwd} failed: ${ran.stderr.toString()}`);
  }
...
beforeAll(async () => {
  ...
  await sh(scratch, 'git', 'clone', '--branch', 'main', '--single-branch', origin, clone);
  ...
  await sh(scratch, 'git', 'clone', '--branch', 'main', '--single-branch', origin, unmarked);
});
...
describe('the marker is the guard (G2)', () => {
  ...
  test('a marker that is not the marker init writes is no marker', async () => {
    ...
    await sh(scratch, 'git', 'clone', '--branch', 'main', '--single-branch', origin, bogus);
...
describe('a marked clone that falls behind (G2)', () => {
  test('the update flow runs exactly once per new head', async () => {
    ...
    for (let index = 0; index < 4; index += 1) await watcher.tick();
    expect(runs.length).toBe(1);
  ...
  test('an update that throws is reported, and not retried on the same head', async () => {
    ...
    await sh(scratch, 'git', 'clone', '--branch', 'main', '--single-branch', origin, behind);
...
describe('a diverged clone refuses loudly (G3)', () => {
  ...
  test('divergence is detected, named, and never resolved by resetting', async () => {
    ...
    await sh(scratch, 'git', 'clone', '--branch', 'main', '--single-branch', origin, diverged);
    ...
    await Bun.write(join(diverged, '.git', 'info', 'exclude'), `${DEPLOY_MARKER}\n`);
    ...
    if (tick.state.kind !== 'diverged') throw new Error(`expected diverged, got ${tick.state.kind}`);
    ...
    expect(await Bun.file(join(diverged, 'local-only.txt')).exists()).toBe(true);
  ...
  test('the trigger itself refuses any state but `behind`', async () => {
    const states: DeployState[] = [
      ...
      { kind: 'ahead', root: '/tmp/nope', head: 'abc1234', target: 'def5678', ahead: 2 },
      {
        kind: 'diverged',
        root: '/tmp/nope',
        head: 'abc1234',
    ...
    for (const state of states) {
      expect(() => deployUpdate(state)).toThrow(`refusing to update a checkout that is ${state.kind}`);
    }
  });

  test('a clone whose remote has gone is unreachable, not current', async () => {
    const orphaned = join(scratch, 'orphaned');
    await sh(scratch, 'git', 'clone', '--branch', 'main', '--single-branch', origin, orphaned);
    await writeMarker(orphaned);
    await sh(orphaned, 'git', 'remote', 'set-url', 'origin', join(scratch, 'gone.git'));
