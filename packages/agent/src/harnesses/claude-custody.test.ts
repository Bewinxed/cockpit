...
const fixture = readFileSync(join(import.meta.dir, 'fixtures', 'custody-control.ndjson'), 'utf8')
  .split('\n')
  .filter((line) => line.trim() && !line.startsWith('#'));

const line = (marker: string): string => {
  const found = fixture.find((entry) => entry.includes(marker));
  if (!found) throw new Error(`fixture has no line matching ${marker}`);
  return found;
};

interface Recorded {
  frames: NeutralMessage[];
  permissions: { requestId: string; toolName: string; input: Record<string, unknown> }[];
  sessions: string[];
  busy: boolean[];
  ctx: HarnessContext;
}
...
const recorder = (): Recorded => {
  const rec: Partial<Recorded> = { frames: [], permissions: [], sessions: [], busy: [] };
  rec.ctx = {
    instanceId: 'inst-1',
    cwd: '/tmp',
    frame: (message) => rec.frames!.push(message),
    permission: (request) => rec.permissions!.push(request as Recorded['permissions'][number]),
...
test('custody re-derives frames from ring lines through the pure toNeutral', () => {
  ...
  expect(r.rec.frames.map((frame) => frame.type)).toEqual(['system', 'assistant']);
  // `toNeutral` is a re-tag: the SDK frame, plus its own self under `raw`.
  const assistant = r.rec.frames[1] as unknown as { uuid: string; raw: { uuid: string } };
...
test('a parked permission is re-parked by requestId and answered with a raw control_response', () => {
  ...
  expect(r.rec.permissions).toEqual([
    { requestId: 'req-perm-1', toolName: 'Bash', input: { command: 'rm -rf build' } },
  ]);
  expect(r.custody.parked).toEqual(['req-perm-1']);

  const result: PermissionResult = { behavior: 'allow', updatedInput: { command: 'rm -rf build' } };
  ...
  expect(() => r.custody.resolvePermission('req-perm-1', result)).toThrow('no permission request');
  expect(r.written).toHaveLength(1);
});
...
test('a NON-permission control during custody fails visibly, in band', () => {
  ...
  const frame = r.rec.frames[0] as unknown as { subtype: string; control: string; text: string };
...
test('an agent-side control during custody rejects, and says so in the transcript', async () => {
  ...
  const frame = r.rec.frames.at(-1) as unknown as { subtype: string; control: string };
  expect(frame.subtype).toBe(CUSTODY_DEGRADED);
...
test('a turn sent during custody is held and delivered by the hand-off', () => {
  const r = rig();
  r.custody.ingest(line('"subtype":"init"'));
  const message = { type: 'user', message: { role: 'user', content: 'carry on' } } as unknown as NeutralUserMessage;
...
test('a stop during custody answers every parked ask rather than leaving it hanging', async () => {
  ...
  expect(r.custody.parked).toEqual([]);
  const answer = JSON.parse(r.written[0]!) as { response: { subtype: string; request_id: string } };
  expect(answer.response.subtype).toBe('error');
 ...
 */
test(
  'custody survives the owner being torn down and rebuilt, and replays without gap or duplicate',
  async () => {
    ...
    try {
      ...
      const textOf = (frame: NeutralMessage): string =>
        ((frame as unknown as { message?: { content?: { text?: string }[] } }).message?.content?.[0]?.text) ?? '';

      // ---- the owner, first life ---------------------------------------
      const first = new ClaudeHarness();
      const firstClient = await first.sessiond(endpoint);
      await firstClient.spawnProc('inst-live', { command: process.execPath, args: [script] });

      const seen: NeutralMessage[] = [];
      const recA = recorder();
      recA.ctx.frame = (frame) => seen.push(frame);
      await first.adopt('inst-live', recA.ctx, { afterSeq: 0, onHandoff: () => {} });
      while (seen.length < 5) await Bun.sleep(10);
      ...
      expect(secondClient.procs.find((proc) => proc.procId === 'inst-live')?.alive).toBe(true);

      const replayed: NeutralMessage[] = [];
      const recB = recorder();
      recB.ctx.frame = (frame) => replayed.push(frame);
      await second.adopt('inst-live', recB.ctx, { afterSeq: cursor, sessionId: 'ses-live', onHandoff: () => {} });
      while (replayed.length < 10) await Bun.sleep(10);
