...
import { hashText, readSidecar, syncMemory, syncSkillFiles, writeJson } from './fleet-common';
...
const modelIdOf = (model: Model<any>): string => String((model as { id?: unknown }).id ?? '');
  ...
  return [
    ...
    defineTool({
      ...
      execute: async (_id, params) => {
        const p = params as { target: string; message: string; urgent?: boolean };
    ...
    defineTool({
      ...
      execute: async (_id, params) => {
        const p = params as { cwd: string; prompt: string; sideQuest?: boolean; model?: string };
        const result = await actions.startSession(p.cwd, p.prompt, p.sideQuest, p.model);
        return answer(result.text, { instanceId: result.id, title: result.title });
      },
    }),
    defineTool({
      name: 'delegate',
    ...
    defineTool({
      ...
      execute: async (_id, params) =>
        answer(await actions.stopDelegate((params as { target: string }).target)),
    }),
    defineTool({
      ...
      parameters: Type.Object({ target: Type.String() }),
      execute: async (_id, params) =>
        answer(await actions.interruptDelegate((params as { target: string }).target)),
    }),
    defineTool({
...
class PiSession implements HarnessSession {
    ...
    switch (event.type) {
      case 'message_update': {
        ...
        switch (ae.type) {
          case 'text_delta': {
            ...
            this.#ctx.frame({ type: 'stream_event', event: { type: 'content_block_delta', delta: { type: 'text_delta', text: delta } } });
            this.#ctx.busy(true);
            this.#busy = true;
            break;
          }
          ...
          case 'toolcall_end': {
            const call = (ae as { toolCall?: { id?: string; name?: string; arguments?: Record<string, unknown> } }).toolCall;
            if (!call?.id || !call.name) break;
            this.#ctx.frame({
              type: 'assistant',
              message: { content: [{ type: 'tool_use', id: call.id, name: call.name, input: call.arguments ?? {} }] },
            });
            this.#ctx.busy(true);
            this.#busy = true;
            break;
      ...
      case 'tool_execution_end': {
        const { toolCallId, toolName, result, isError } = event as unknown as {
          toolCallId: string;
          toolName: string;
          result: { content?: { type?: string; text?: string }[]; details?: Record<string, unknown> } | unknown;
          isError: boolean;
        };
        const content = Array.isArray(result) ? '' : textOf((result as { content?: unknown })?.content) || '';
        const details = (result as { details?: Record<string, unknown> } | null)?.details;
        const structuredContent = details && Object.keys(details).length > 0 ? details : undefined;
        this.#ctx.frame({
          type: 'user',
          message: {
            role: 'user',
      ...
      case 'turn_end': {
        ...
        for (const block of message?.content ?? []) {
          ...
          else if (b.type === 'thinking' && b.thinking) blocks.push({ type: 'thinking', thinking: b.thinking });
        }
        if (blocks.length) this.#ctx.frame({ type: 'assistant', message: { content: blocks } });
        else if (this.#streamedText) this.#ctx.frame({ type: 'assistant', message: { content: [{ type: 'text', text: this.#streamedText }] } });
        else if (this.#streamedThinking) this.#ctx.frame({ type: 'assistant', message: { content: [{ type: 'thinking', thinking: this.#streamedThinking }] } });
        break;
      }
      case 'agent_end': {
        this.#streamedText = '';
        ...
        const failed = (event as { messages?: { role?: string; errorMessage?: string }[] }).messages?.some(
  ...
  send(message: NeutralUserMessage, extras: Pick<SendPayload, 'attachments' | 'images' | 'urgent'>): void {
    ...
    const images = (extras.images ?? []).map((image) => ({ type: 'image' as const, data: image.data, mimeType: image.mediaType }));
    ...
    if (extras.urgent && this.#busy) {
      ...
      void this.#session
        ...
        .then(() => {
          this.#ctx.busy(true);
          this.#busy = true;
          void this.#session.prompt(prompt, { images: images as never }).catch((error) => this.#ctx.failed(error));
        });
      return;
    }
    ...
    this.#ctx.busy(true);
    this.#busy = true;
    void this.#session.prompt(text + attachments, { images: images as never }).catch((error) => this.#ctx.failed(error));
  }
  ...
  async control(method: string, args: unknown[]): Promise<unknown> {
    switch (method) {
      ...
      case CONTROL_SET_MODEL: {
        ...
        if (!model) throw new Error(`pi does not know model ${args[0]}`);
        await this.#session.setModel(model);
        return undefined;
      }
      case CONTROL_CONTEXT_USAGE: {
        const last = [...this.#session.messages].reverse().find((m) => (m as { usage?: unknown }).usage);
        const usage = (last as { usage?: { totalTokens?: number } })?.usage;
        const total = usage?.totalTokens ?? 0;
        return {
          totalTokens: total,
          maxTokens: 200000,
          percentage: Math.min(100, Math.round((total / 200000) * 100)),
          categories: [],
        };
      }
      default:
  ...
  async #resolveModel(id: string): Promise<Model<any> | undefined> {
    ...
    return (await runtime.getAvailable()).find((model) => modelIdOf(model) === id);
  }
...
export class PiHarness implements Harness {
  ...
  static runtime(): Promise<ModelRuntime> {
    if (!runtimePromise) {
      runtimePromise = ModelRuntime.create({ refreshOnCreate: false }).catch((error) => {
        runtimePromise = null;
        throw error;
      });
    }
  ...
  async #resolveModel(modelId: string): Promise<Model<any> | undefined> {
    const runtime = await PiHarness.runtime();
    return (await runtime.getAvailable()).find((model) => modelIdOf(model) === modelId);
  }
  ...
  async spawn(spec: SpawnPayload, ctx: HarnessContext): Promise<HarnessSession> {
    ...
    if (spec.resume?.fork) {
      ...
      if (source) sessionManager = SessionManager.forkFrom(source, ctx.cwd);
    } else if (spec.resume) {
      const source = await this.#sessionPath(spec.resume.sessionKey, ctx.cwd);
      if (source) sessionManager = SessionManager.open(source, undefined, ctx.cwd);
    }
  ...
  async #sessionPath(sessionKey: string, cwd: string): Promise<string | undefined> {
    const sessions = await SessionManager.list(cwd);
    const found = sessions.find((info) => info.id === sessionKey || info.path.includes(sessionKey));
    return found?.path;
  }
  ...
  async listSessions(dir?: string): Promise<NeutralSessionInfo[]> {
    ...
    return sessions.map((info) => ({
      sessionId: info.id,
      harness: 'pi',
      lastModified: (info as { updatedAt?: number }).updatedAt ?? (info as { createdAt?: number }).createdAt ?? Date.now(),
  ...
  async getSessionInfo(sessionKey: string, dir?: string): Promise<NeutralSessionInfo | undefined> {
    const info = (await this.listSessions(dir)).find((one) => one.sessionId === sessionKey);
    return info;
  }
  ...
  async getSessionMessages(sessionKey: string, dir?: string): Promise<SessionMessage[]> {
    ...
    for (const entry of manager.getEntries()) {
      ...
      const message = (entry as { message?: unknown }).message as { role?: string; content?: unknown } | undefined;
      if (!message) continue;
      ...
      if (role === 'user') {
        ...
        entries.push({
          ...
          message: {
            role: 'user',
            content: [{ type: 'tool_result', tool_use_id: tool.toolCallId ?? '', content: textOf(message.content), is_error: tool.isError }],
          },
          parent_tool_use_id: null,
          parent_agent_id: null,
        });
  ...
  async deleteSession(sessionKey: string, dir?: string): Promise<void> {
    if (!dir) return;
    const path = await this.#sessionPath(sessionKey, dir);
    if (path) await Bun.$`rm -f ${path}`.quiet().nothrow();
  }
  ...
  async syncFleet(config: FleetConfig): Promise<FleetSyncReport> {
    ...
    const skills = await syncSkillFiles(PI_SKILLS, config.skills ?? [], sidecar.skills, report.skills!);
  ...
  async fleetStatus(): Promise<FleetSyncReport> {
    ...
    for (const name of Object.keys(sidecar.skills)) {
      ...
      report.skills![name] = (await file.exists()) ? { state: 'applied' } : { state: 'failed', detail: 'not on disk' };
    }
...
function toBlocks(content: unknown): NeutralAssistantBlock[] {
  ...
  for (const block of content) {
    ...
    else if (b.type === 'toolCall' && b.id && b.name) blocks.push({ type: 'tool_use', id: b.id, name: b.name, input: b.arguments ?? {} });
  }
  return blocks;
}
