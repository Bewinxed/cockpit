...
const startDaemon = async (): Promise<string> => {
  ...
  while (Date.now() < deadline) {
    if (await probeEndpoint(endpoint, 200)) return endpoint;
    await Bun.sleep(25);
  }
...
test('the announced port is parsed agent-side, from the format the binary actually prints', () => {
  ...
  expect(
    parseServerAnnouncement('Warning: OPENCODE_SERVER_PASSWORD is not set; server is unsecured.')
  ).toBeUndefined();
  expect(parseServerAnnouncement('opencode server listening on http://127.0.0.1:43663')).toBe(
    'http://127.0.0.1:43663'
  );
...
test(
  'the server is spawned through sessiond, its port read from the ring, and one server is kept',
  async () => {
    ...
    const url = await attachOpencodeServer({ sessiond: client, spec: serverSpec(dir) });
    ...
    const again = await attachOpencodeServer({ sessiond: client, spec: serverSpec(dir) });
...
test(
  'a torn-down agent side rebuilds onto the still-running server and re-subscribes SSE',
  async () => {
    const endpoint = await startDaemon();
    const dir = scratch();
    ...
    const url = await attachOpencodeServer({ sessiond: first, spec: serverSpec(dir) });
    ...
    expect(await (await fetch(`${url}/subscriptions`)).json()).toEqual({ subscriptions: 1 });
    ...
    const rebuilt = await attachOpencodeServer({ sessiond: second, spec: serverSpec(dir) });
