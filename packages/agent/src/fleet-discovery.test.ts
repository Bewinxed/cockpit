...
test("a name in two scopes: the nearer wins and the further says so", () => {
  const found = mergeMcp(
    [
      ...
      {
        scope: "project",
        servers: { linear: stdio("project-linear"), repo: stdio("repo") },
      },
      {
        scope: "user",
        servers: { linear: stdio("user-linear"), exa: stdio("exa") },
      },
    ],
    []
  );
  ...
  expect(linear.map((row) => row.shadowedBy)).toEqual([
    undefined,
    "local",
    "local",
  ]);
...
test("only the user scope can be whiffle's, and only what the sidecar names", () => {
  const found = mergeMcp(
    [
      ...
      {
        scope: "user",
        servers: { exa: stdio("user-exa"), mine: stdio("mine") },
      },
    ],
    ["exa"]
  );
  ...
  expect(
    found.find((row) => row.scope === "user" && row.name === "exa")?.managed
  ).toBe(true);
...
test("a plugin names its skills the way a session calls them", async () => {
  ...
  await Bun.write(
    join(installPath, "skills", "better-colors", "SKILL.md"),
    "# No front matter\n"
  );
  // A directory without a SKILL.md is not a skill, whatever else is in it.
  await Bun.write(
    join(installPath, "skills", "notes", "README.md"),
    "not a skill\n"
  );
  ...
  expect(roots).toEqual([
    { plugin: "interfaces", root: join(installPath, "skills") },
  ]);
  ...
  expect(
    found.find((skill) => skill.name === "interfaces:better-ui")?.description
  ).toBe("Details that make interfaces feel better.");
  expect(
    found.find((skill) => skill.name === "interfaces:better-colors")
      ?.description
  ).toBeUndefined();
});
...
test("a plugin that files its skills under categories still names them plainly", async () => {
  ...
  await Bun.write(
    join(root, "engineering", "tdd", "SKILL.md"),
    "---\nname: tdd\n---\n"
  );
  await Bun.write(
    join(root, "productivity", "grilling", "SKILL.md"),
    "---\nname: grilling\n---\n"
  );
