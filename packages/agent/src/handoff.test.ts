...
const rows = [
  ...
  { id: 'router', machineId: 'm1', cwd: '/home/o/locallm-router', status: 'running' },
...
type Handler = (args: Record<string, string>, extra: unknown) => Promise<{ content: unknown[] }>;
...
test('a hand-off emits a send addressed at the target and its machine', async () => {
  ...
  await handoff({ target: 'keeboard', message: 'auth double-encodes the token' }, {});
...
test('the message is marked as a peer and does not start a turn', async () => {
  ...
  const message = payload.message as {
    origin?: { kind: string; from: string; name?: string; fromSession?: string };
    shouldQuery?: boolean;
...
test('an unknown target is refused, and says what is running', async () => {
  const { handoff, sent } = build();
  expect(handoff({ target: 'nowhere', message: 'x' }, {})).rejects.toThrow(/keeboard/);
...
test('starting a session spawns it and gives it its opening turn', async () => {
  ...
  await start(
    { cwd: '/home/o/center.ai', prompt: 'audit the extension manifest' } as never,
    {}
  );
  ...
  const message = (opening.payload as { message: Record<string, unknown> }).message;
...
test('a side quest is spawned as scratch, with its worktree flag', async () => {
  ...
  const scratch = (sent[0].payload as { scratch?: { worktree?: boolean; baseCwd?: string } }).scratch;
