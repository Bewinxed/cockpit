...
test('the session running a model is shown that model’s document', async () => {
  expect(await shown('{"session_id":"s","hook_event_name":"SessionStart","model":"claude-opus-5"}')).toBe(
    'opus guidance\n'
  );
});
...
test('the model as an object, which is the other shape it arrives in', async () => {
  expect(
    await shown('{"model":{"id":"claude-opus-5","display_name":"Opus 5"},"source":"startup"}')
  ).toBe('opus guidance\n');
});

test('longest prefix wins, so a dated id still finds its own document', async () => {
  expect(await shown('{"model":"claude-opus-5-20260315"}')).toBe('opus guidance\n');
