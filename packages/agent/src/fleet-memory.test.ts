...
test("an unmanaged file that says something else is never clobbered", () => {
  ...
  expect(memoryPlan({ hash: FLEET, force: true }, MINE, undefined)).toBe(
    "write"
  );
});
...
test("one document edited on the machine holds up itself and nothing else", () => {
  ...
  expect(memorySetPlan(desired, onDisk, managed)).toEqual({
    [OPUS]: "drift",
    [FABLE]: "skip",
  });
  expect(
    memorySetPlan(
      desired.map((doc) => ({ ...doc, force: true })),
      onDisk,
      managed
    )
  ).toEqual({
...
test("a document only the sidecar still names is given back, edited ones kept", () => {
  expect(memorySetPlan([], { [OPUS]: FLEET }, { [OPUS]: FLEET })).toEqual({
    [OPUS]: "remove",
  });
  expect(memorySetPlan([], { [OPUS]: MINE }, { [OPUS]: FLEET })).toEqual({
    [OPUS]: "unmanage",
  });
});
...
test("a daemon that predates the set has no managed documents to lose", () => {
  ...
  expect(memorySetPlan([{ path: OPUS, hash: FLEET }], {}, {})).toEqual({
    [OPUS]: "write",
  });
});
...
test("a document somebody wrote by hand is never taken over silently", () => {
  expect(
    memorySetPlan([{ path: OPUS, hash: FLEET }], { [OPUS]: MINE }, {})
  ).toEqual({
    [OPUS]: "drift",
  });
  expect(
    memorySetPlan(
      [{ path: OPUS, hash: FLEET, force: true }],
      { [OPUS]: MINE },
      {}
    )
  ).toEqual({
...
test("removing the hook leaves every other hook where it is", () => {
  const theirs = {
    hooks: [{ type: "command", command: "/home/x/bin/notify" }],
  };
  expect(
    withoutHooks(
      [theirs, { hooks: [{ type: "command", command: OURS }] }],
      [OURS]
    )
  ).toEqual([theirs]);
});
  ...
  expect(
    withoutHooks(
      [
        {
          matcher: "startup",
          hooks: [{ command: OURS }, { command: "/home/x/bin/notify" }],
        },
      ],
      [OURS]
    )
  ).toEqual([
    { matcher: "startup", hooks: [{ command: "/home/x/bin/notify" }] },
  ]);
});
