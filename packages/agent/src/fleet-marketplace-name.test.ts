...
import { type KnownMarketplace, linkedNameIn, toUnlink, upgradeMarketplaces } from './fleet';
 ...
 */
const known = (source: KnownMarketplace['source']): KnownMarketplace => ({ source });
...
test("a manifest name that is not the repo name resolves through the source", () => {
  ...
  expect(linkedNameIn(rtd, 'rtd-claude-inn', 'ryanthedev/rtd-claude-inn')).toBe('rtd');
});
...
test('a mixed sidecar upgrades the strings and leaves the rest alone', () => {
  const linked = { ...rtd, interfaces: known({ source: 'github', repo: 'jakubkrehel/skills' }) };
  expect(
    upgradeMarketplaces(['rtd-claude-inn', 'interfaces', { name: 'x', linkedAs: 'rtd' }], linked)
