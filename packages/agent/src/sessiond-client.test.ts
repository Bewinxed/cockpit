...
test(
  'a child keeps running and its ring keeps filling while the agent side is torn down and rebuilt',
  async () => {
    ...
    await first.spawnProc(procId, { command: process.execPath, args: [emitter(dir)] });

    const seen: { seq: number; n: number }[] = [];
    first.subscribe(procId, { line: (event) => seen.push({ seq: event.seq, n: nOf(event.data) }) }, 0);
    ...
    const exited = new Promise<void>((resolve) => {
      second.subscribe(procId, { line: () => {}, exit: () => resolve() }, replayed.at(-1)!.seq);
    });
    await second.stdinEnd(procId);
    await Promise.race([exited, Bun.sleep(5_000)]);
    expect((await second.list()).procs.find((proc) => proc.procId === procId)?.alive).toBe(false);
  },
  20_000
);
...
test('an unknown verb is answered, never fatal — the §5 capability probe', async () => {
  ...
  // connection.
  const raw = client as unknown as { spawnProc: (id: string, spec: unknown) => Promise<void> };
  await expect(raw.spawnProc('bad', {})).rejects.toThrow('spawn: procId and spec.command required');
...
test('under service management a missing sessiond is a loud error, NEVER an ad-hoc spawn', async () => {
  ...
  await expect(ensureSessiond(endpoint, { INVOCATION_ID: 'unit-1' })).rejects.toBeInstanceOf(
    SessiondUnavailableError
  );
